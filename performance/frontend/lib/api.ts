...
api.interceptors.request.use(
  (config) => {
    // Get token from localStorage (set by OIDC callback)
  ...
  (error: AxiosError) => {
    if (error.response?.status === 401) {
      ...
      if (typeof window !== 'undefined') {
        ...
        // 2. OIDC callback in progress (token in URL hash)
        // 3. No token in storage (user was never logged in, will be handled by AuthContext)
        if (!isLoginPage && !hasTokenInHash && hasTokenInStorage) {
