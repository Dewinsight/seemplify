...
function secret(environment = process.env) {
  const file = String(environment.IDP_PERFORMANCE_PLATFORM_INTEGRATION_HMAC_SECRET_FILE || environment.IDP_PLATFORM_INTEGRATION_HMAC_SECRET_FILE || '').trim();
  const value = file ? fs.readFileSync(file, 'utf8').trim() : String(environment.IDP_PERFORMANCE_PLATFORM_INTEGRATION_HMAC_SECRET || environment.IDP_PLATFORM_INTEGRATION_HMAC_SECRET || '').trim();
  if (value.length >= 32) return value;
  if (environment.NODE_ENV !== 'production') return 'experience-admin-development-secret-change-me';
  throw new Error('Performance storage configuration authentication is not configured.');
}
...
async function resolveStoragePlatformConfiguration({ environment = process.env, force = false } = {}) {
  ...
  cache = { value: configuration, expiresAt: now + 5 * 60_000 };
