...
function defaultReceiptModel() {
  // Keep the pure receipt state machine testable without loading Mongoose.
  // Production resolves the model only when the route actually claims an event.
...
async function claimIdpWebhookEvent(payload, {
  ...
  if (!eventId) {
    throw Object.assign(new Error('Webhook eventId is required'), { code: 'IDP_EVENT_ID_REQUIRED' });
  }
