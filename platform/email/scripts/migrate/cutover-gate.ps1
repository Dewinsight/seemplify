...
$check=Invoke-MailRemote -Remote $remote -Script "'$RemoteRoot/bin/remote-readiness.sh' --project '$RemoteProject' --require-active --public-url '$($StagingBaseUrl.AbsoluteUri.TrimEnd('/'))' --json" -AllowFailure
if($check.ExitCode -ne 0){throw "Remote readiness failed: $($check.Output)"}
$queues=Invoke-MailRemote -Remote $remote -Script "'$RemoteRoot/bin/queue-inspect.sh' --project '$RemoteProject' --require-empty --json" -AllowFailure
