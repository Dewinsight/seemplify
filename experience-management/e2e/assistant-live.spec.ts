...
test('real assistant backend completes OAuth, reads a thread, and durably saves Terra summary and draft work', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Connected mailbox' }).locator('option:checked')).toContainText('connected@example.test');
  ...
  await expect(draft).toContainText(/confirm by Friday/u);
