...
export const DialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, DialogContentProps>(({ className, children, showCloseButton = true, ...props }, ref) => (
  <DialogPortal><DialogOverlay /><DialogPrimitive.Content ref={ref} className={cn('fixed left-3 right-auto top-3 z-50 isolate grid max-h-[calc(100dvh-1.5rem)] min-w-0 w-[calc(100vw-1.5rem)] max-w-[calc(100vw-1.5rem)] gap-4 overflow-y-auto overscroll-contain rounded-lg border bg-background p-5 shadow-panel focus:outline-none sm:left-1/2 sm:top-1/2 sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)] sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:p-6', className)} {...props}>{children}{showCloseButton && <DialogPrimitive.Close aria-label="Close" className="absolute right-2 top-2 z-20 flex h-10 w-10 items-center justify-center rounded-md bg-background/95 text-muted-foreground hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 sm:right-3 sm:top-3"><X className="h-4 w-4" /></DialogPrimitive.Close>}</DialogPrimitive.Content></DialogPortal>
));
