...
export function FieldProperties({ field, recipients, onChange, onDelete }: { field?: ESignField; recipients: ESignRecipient[]; onChange: (field: ESignField) => void; onDelete: () => void }) {
  if (!field) return <aside className="hidden w-[300px] shrink-0 border-l bg-card p-5 xl:block"><div className="text-sm font-semibold">Field properties</div><p className="mt-2 text-xs leading-5 text-muted-foreground">Select a field on the document to edit its assignment, size and validation.</p></aside>;
  const selectedField = field;
  function change(values: Partial<ESignField>) { onChange({ ...selectedField, ...values }); }
  return <aside className="w-full shrink-0 border-t bg-card p-4 xl:w-[300px] xl:border-l xl:border-t-0 xl:p-5"><div className="flex items-center justify-between"><div className="text-sm font-semibold">Field properties</div><Button variant="ghost" size="icon" aria-label="Delete selected field" onClick={onDelete}><Trash2 /></Button></div><div className="mt-4 space-y-4">
