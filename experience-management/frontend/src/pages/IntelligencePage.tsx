    ...
    <div className="grid min-w-0 items-start gap-5 xl:grid-cols-[292px_minmax(0,1fr)]">
      ...
      {selected && <div className="border bg-card">
        ...
        <div className="space-y-5 p-4">
          ...
          <dl className="grid gap-x-6 gap-y-3 border-y py-3 text-xs sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-muted-foreground">Estimated input</dt><dd className="mt-1 font-medium">{Number(estimate.estimatedInputTokens || 0).toLocaleString()} tokens</dd></div><div><dt className="text-muted-foreground">Planned calls</dt><dd className="mt-1 font-medium">{Number(estimate.estimatedCalls || 0).toLocaleString()}</dd></div><div><dt className="text-muted-foreground">Corpus partitions</dt><dd className="mt-1 font-medium">{Number(estimate.mapPartitions || 0).toLocaleString()} text · {Number(estimate.graphPartitions || 0).toLocaleString()} graph</dd></div><div><dt className="text-muted-foreground">Initial estimate</dt><dd className="mt-1 font-medium">{duration ? `About ${duration} min` : 'Calculating'}</dd></div></dl>
...
function ReportSources({ refs, sourceMap, report, totalTokens }: {
  ...
  return <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_280px]">
    ...
    <aside><h3 className="text-sm font-semibold">Run provenance</h3><dl className="mt-3 divide-y border-y text-xs"><div className="flex justify-between gap-3 py-2.5"><dt className="text-muted-foreground">Runtime</dt><dd className="text-right font-medium">{report.runtime?.provider || report.runtime?.model || 'ChatGPT Connect'}</dd></div><div className="flex justify-between gap-3 py-2.5"><dt className="text-muted-foreground">Completed</dt><dd className="text-right font-medium">{formatDate(report.completedAt)}</dd></div>{totalTokens ? <div className="flex justify-between gap-3 py-2.5"><dt className="text-muted-foreground">Tokens</dt><dd className="text-right font-medium">{totalTokens.toLocaleString()}</dd></div> : null}{report.runtime?.latencyMs ? <div className="flex justify-between gap-3 py-2.5"><dt className="text-muted-foreground">Latency</dt><dd className="text-right font-medium">{report.runtime.latencyMs.toLocaleString()} ms</dd></div> : null}</dl></aside>
  </div>;
}
