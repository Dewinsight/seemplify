...
function RuntimeWorkspace({ payload, loading, error, refresh }: { payload: RuntimePayload | null; loading: boolean; error: string; refresh: () => Promise<void> }) {
  ...
  const azureActive = runtime?.activeEmbeddingProvider === 'azure-openai';
  const serviceRows = [
    ['arango', 'ArangoDB', 'Documents, vectors, graph, and provenance'],
    azureActive
      ? ['embedding', 'Azure embedding service', 'text-embedding-3-large for indexing and retrieval']
      : ['gteEmbedding', 'GTE embedding worker', 'CPU embeddings for indexing and retrieval'],
    ['reranker', azureActive ? 'Azure cosine reranker' : 'BGE reranker', 'Reorders retrieved evidence'],
    ['docling', 'Docling', 'Document extraction and OCR'],
    ['chatgpt', 'ChatGPT gateway', 'Graph extraction and grounded answers']
  ] as const;
  const profile = runtime?.embeddingProfiles?.[runtime?.activeEmbeddingProvider || ''] || runtime?.gte?.profile || {};
  const migration = runtime?.migration || {};
  const vectorIndexes = runtime?.search?.vectorIndexes || {};
  const provider = runtime?.providers?.[runtime?.activeEmbeddingProvider || 'gte-node'] || {};
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="border bg-card"><header className="border-b px-4 py-3"><h2 className="text-sm font-semibold">Embedding</h2><p className="mt-1 text-xs text-muted-foreground">The active vector-space contract and embedding service.</p></header><dl className="divide-y text-sm"><RuntimeDetail label="Provider" value={runtime?.activeEmbeddingProvider || '—'} /><RuntimeDetail label="Model" value={runtimeText(profile, 'model', runtimeText(profile, 'modelId'))} /><RuntimeDetail label="Revision" value={runtimeText(profile, 'revision')} mono /><RuntimeDetail label="Execution" value={`${runtimeText(profile, 'execution', azureActive ? 'azure-openai' : 'local')} · ${runtimeText(profile, 'dtype')} · ${runtimeText(profile, 'dimensions', runtimeText(profile, 'dimension'))} dimensions`} /><RuntimeDetail label="Index version" value={runtimeText(profile, 'vectorIndexVersion')} />{!azureActive && <RuntimeDetail label="Worker" value={`${runtime?.gte?.state || 'unknown'} · accepting ${runtime?.gte?.accepting ? 'yes' : 'no'} · generation ${runtimeText(runtime?.gte?.worker, 'generation')}`} />}<RuntimeDetail label="Provider activity" value={`${runtimeText(provider, 'embeddings', '0')} requests · ${runtimeText(provider, 'texts', '0')} texts · ${runtimeText(provider, 'failures', '0')} failures`} /><RuntimeDetail label="Qwen activity" value={`${runtimeText(qwen, 'embeddings', '0')} requests · rollback ${runtimeText(migration, 'dualWrite', 'false') === 'true' ? 'retained' : 'retired'}`} /></dl></section>

      <section className="border bg-card"><header className="border-b px-4 py-3"><h2 className="text-sm font-semibold">Queue and workers</h2><p className="mt-1 text-xs text-muted-foreground">Runtime retrieval work and durable application indexing.</p></header><dl className="divide-y text-sm"><RuntimeDetail label="Runtime queue" value={`${runtime?.queue?.waiting || 0} waiting · ${runtime?.queue?.accepting ? 'accepting work' : 'paused'}`} /><RuntimeDetail label="Active work" value={`${activeQueue.retrieve || 0} retrieval · ${activeQueue.graph || 0} graph`} /><RuntimeDetail label="Runtime totals" value={`${runtime?.queue?.completed || 0} completed · ${runtime?.queue?.failed || 0} failed`} /><RuntimeDetail label="Oldest wait" value={`${runtime?.queue?.oldestWaitMs || 0} ms`} /><RuntimeDetail label="Indexing worker" value={`${runtimeText(appWorker, 'active', '0')} active · ${runtimeText(appWorker, 'queued', '0')} queued · concurrency ${runtimeText(appWorker, 'concurrency', '1')}`} />{!azureActive && <RuntimeDetail label="GTE queue" value={`${runtimeText(runtime?.gte?.queue, 'waiting', '0')} waiting · capacity ${runtimeText(runtime?.gte?.queue, 'capacity')}`} />}</dl></section>
...
function KnowledgeChatWorkspace({ knowledgeBase, additionalIds, setAdditionalIds, question, setQuestion, messages, working, ready, ask, openSettings }: {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
    <section className="border bg-card">
      ...
      {!knowledgeBase.terraContextEnabled && <div className="flex flex-col justify-between gap-3 border-b bg-amber-50 px-4 py-3 text-sm text-amber-950 sm:flex-row sm:items-center"><span>AI context is off. Enable it before authorised document text can be sent to connected ChatGPT.</span><Button size="sm" variant="outline" onClick={openSettings}>Open settings</Button></div>}
    ...
    <aside className="space-y-4">
      ...
      <section className="border bg-card px-4 py-3 text-xs leading-5 text-muted-foreground"><div className="font-semibold text-foreground">How grounding works</div><p className="mt-1">The configured embedding service retrieves passages and reranks them by semantic similarity. Terra receives only the selected evidence and must return inline source references.</p></section>
    </aside>
  </div>;
}
