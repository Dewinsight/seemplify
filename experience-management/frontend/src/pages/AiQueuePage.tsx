...
export function AiQueuePage() {
  ...
  const metrics = [['Queued', counts.queued], ['Processing', counts.processing], ['Completed', counts.completed], ['Failed', counts.failed], ['Worker slots', `${runtime?.worker?.active || 0}/${runtime?.worker?.concurrency || 1}`], ['Terra', runtime?.terra?.reachable ? 'Ready' : 'Offline']];
  return <div className="space-y-6">
    <div><h1 className="page-title">Terra AI queue</h1><p className="page-description">Live and historical view of every Experience AI request. A single request uses the same durable queue as a burst.</p></div>
    ...
    <div className="grid gap-4 border bg-card p-5 text-sm md:grid-cols-3"><div className="flex gap-3"><Activity className="h-4 w-4 text-primary" /><div><div className="font-medium">Durable dispatch</div><div className="mt-1 text-xs leading-5 text-muted-foreground">Jobs survive application restarts and retry while Terra is offline.</div></div></div><div className="flex gap-3"><Cpu className="h-4 w-4 text-primary" /><div><div className="font-medium">Local-cloud only</div><div className="mt-1 text-xs leading-5 text-muted-foreground">Experience activities are locked to Terra with no provider fallback.</div></div></div><div className="flex gap-3"><Server className="h-4 w-4 text-primary" /><div><div className="font-medium">Signed gateway</div><div className="mt-1 text-xs leading-5 text-muted-foreground">Every execution uses a timestamped HMAC request and durable metering identity.</div></div></div></div>
  </div>;
}
