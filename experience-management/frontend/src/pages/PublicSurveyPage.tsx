...
export function PublicSurveyPage() {
  ...
  const [data, setData] = useState<{ survey: Survey; collector: Collector } | null>(null);
  ...
  return <main className="min-h-screen bg-background"><header className="border-b bg-card"><div className="mx-auto flex h-16 max-w-3xl items-center justify-between px-5"><div className="text-sm font-semibold">{data.survey.theme?.brandName || 'Seemplify Experience'}</div><div className="text-xs text-muted-foreground">Page {page} of {pages}</div></div></header><div className="h-1 bg-muted"><div className="h-full bg-primary transition-all" style={{ width: `${(page / pages) * 100}%` }} /></div><form onSubmit={submit} className="mx-auto max-w-3xl px-5 py-10 sm:py-14"><div className="mb-10"><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">{data.survey.title}</h1>{page === 1 && data.survey.description && <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">{data.survey.description}</p>}</div><div className="space-y-9">{questions.map((question) => <section key={question.id}><label className="mb-3 block text-base font-medium"><span className="mr-2 text-sm text-muted-foreground">{(data.survey.questions || []).indexOf(question) + 1}.</span>{question.title}{question.required && question.type !== 'statement' && <span className="ml-1 text-destructive">*</span>}</label>{question.description && question.type !== 'statement' && <p className="-mt-2 mb-3 text-sm text-muted-foreground">{question.description}</p>}<QuestionInput question={question} value={answers[question.id]} onChange={(value) => setAnswers((current) => ({ ...current, [question.id]: value }))} /></section>)}</div>{missing.length > 0 && <p className="mt-8 text-sm text-destructive" role="alert">Please answer all required questions on this page.</p>}<div className="mt-10 flex justify-between border-t pt-5">{pageHistory.length ? <Button type="button" variant="outline" onClick={back}><ArrowLeft />Back</Button> : <span />}<Button type="submit" disabled={submitting || missing.length > 0}>{submitting ? <Loader2 className="animate-spin" /> : page < pages ? <ArrowRight /> : <Check />}{page < pages ? 'Continue' : submitting ? 'Submitting' : 'Submit response'}</Button></div><div className="mt-8 text-center text-xs text-muted-foreground">Your response is processed according to this survey's collection policy.</div></form></main>;
}
