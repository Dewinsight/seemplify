...
export function CampaignWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!setupDirtyRef.current) { setName(next.campaign.name); setSenderName(next.campaign.senderName || 'Seemplify Experience'); setSelectedSurveyId(next.campaign.surveyId); }
  ...
  const workflowSections = useMemo(() => {
    ...
    if (!senderName.trim()) setupIssues.push('Enter a sender display name.');
  ...
  }, [detail, name, scheduleDirty, selectedSurvey, selectedSurveyId, senderName, sequenceDraft, setupDirty, startAt]);
  ...
  async function saveSetup() {
    ...
    if (!senderName.trim()) return toast.error('Enter a sender display name.');
    if (!selectedSurveyId) return toast.error('Select the survey for this campaign.');
    try {
      setWorking(true);
      await api(`/api/campaigns/${id}`, json('PUT', {
        name: name.trim(),
        ...(detail?.campaign.launchedAt ? {} : { surveyId: selectedSurveyId, senderName: senderName.trim() })
      }));
  ...
  return <div className="space-y-5">
    ...
    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as CampaignWorkspaceTab)}>
      ...
      <TabsContent value="setup"><fieldset disabled={working} className="border-0 p-0"><Card className="max-w-2xl"><CardHeader><CardTitle>Campaign setup</CardTitle><p className="text-xs leading-5 text-muted-foreground">Name the campaign, choose the exact survey and confirm the sender recipients will see.</p></CardHeader><CardContent className="space-y-4"><div><Label className="field-label" htmlFor="campaign-settings-name">Campaign name <span className="text-destructive" aria-hidden="true">*</span></Label><Input id="campaign-settings-name" value={name} required aria-required="true" aria-invalid={name.trim().length < 2} onChange={(event) => { markSetupDirty(); setName(event.target.value); }} />{name.trim().length < 2 && <p className="mt-1 text-xs text-destructive">Enter at least two characters.</p>}</div><div><Label className="field-label" htmlFor="campaign-settings-survey">Survey <span className="text-destructive" aria-hidden="true">*</span></Label><select id="campaign-settings-survey" aria-required="true" aria-invalid={!selectedSurveyId || selectedSurvey?.status !== 'live'} aria-describedby="campaign-settings-survey-help" className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" value={selectedSurveyId} disabled={Boolean(campaign.launchedAt)} onChange={(event) => { markSetupDirty(); setSelectedSurveyId(event.target.value); }}><option value="">Select the survey to send</option>{surveys.map((item) => <option value={item.id} key={item.id}>{item.title} ({item.status})</option>)}</select><p id="campaign-settings-survey-help" className={`mt-1 text-xs leading-5 ${selectedSurvey && selectedSurvey.status !== 'live' ? 'text-destructive' : 'text-muted-foreground'}`}>{campaign.launchedAt ? 'The survey is locked after launch so recipient links and response attribution remain correct.' : !selectedSurvey ? 'Required: choose the exact survey for this campaign.' : selectedSurvey.status !== 'live' ? 'This survey must be published before the setup step is complete.' : 'The selected live survey will be linked in every campaign message.'}</p></div><div className="border-t pt-4"><div className="mb-3"><div className="text-sm font-semibold">Sender identity</div><p className="mt-1 text-xs leading-5 text-muted-foreground">This From identity is used for every email in the sequence.</p></div><div className="grid gap-4 sm:grid-cols-2"><div><Label className="field-label" htmlFor="campaign-sender-name">Sender display name <span className="text-destructive" aria-hidden="true">*</span></Label><Input id="campaign-sender-name" value={senderName} maxLength={150} required aria-required="true" aria-invalid={!senderName.trim()} aria-describedby="campaign-sender-name-help" disabled={campaign.status !== 'draft' || Boolean(campaign.launchedAt)} onChange={(event) => { markSetupDirty(); setSenderName(event.target.value); }} /><p id="campaign-sender-name-help" className={`mt-1 text-xs leading-5 ${!senderName.trim() ? 'text-destructive' : 'text-muted-foreground'}`}>{campaign.status !== 'draft' || campaign.launchedAt ? 'The sender identity is locked after launch.' : !senderName.trim() ? 'Required: enter a sender display name.' : 'Recipients will see this name in their inbox.'}</p></div><div><Label className="field-label" htmlFor="campaign-sender-email">Verified sender email</Label><Input id="campaign-sender-email" type="email" value={campaign.senderEmail} readOnly aria-readonly="true" aria-describedby="campaign-sender-email-help" /><p id="campaign-sender-email-help" className="mt-1 text-xs leading-5 text-muted-foreground">This address is configured and verified for the mail service, so it cannot be changed in a campaign.</p></div></div></div><Button disabled={working || !setupDirty || name.trim().length < 2 || !senderName.trim() || !selectedSurveyId} onClick={() => void saveSetup()}><Save />{working ? 'Saving' : 'Save setup'}</Button></CardContent></Card></fieldset></TabsContent>
      ...
      <TabsContent value="review"><Card className="max-w-3xl"><CardHeader><CardTitle>{campaign.status === 'draft' ? 'Review and launch' : 'Campaign summary'}</CardTitle><p className="text-xs leading-5 text-muted-foreground">{campaign.status === 'draft' ? 'Every required workflow step must be complete. You can return to any step without losing your place.' : 'The campaign setup and delivery choices are retained here for reference.'}</p></CardHeader><CardContent className="space-y-5"><div className="divide-y border">{workflowSections.map((section) => <button type="button" className="flex w-full items-start justify-between gap-4 px-4 py-3 text-left hover:bg-muted/30" key={section.key} onClick={() => setActiveTab(section.key)}><div><div className="text-sm font-medium">{workflowLabels[section.key]}</div><div className={`mt-1 text-xs leading-5 ${section.complete ? 'text-muted-foreground' : 'text-destructive'}`}>{section.complete ? section.key === 'audience' ? campaign.status === 'draft' ? `${detail.contacts.filter((contact) => contact.status === 'active').length} active contact${detail.contacts.filter((contact) => contact.status === 'active').length === 1 ? '' : 's'}` : `${metrics.contacts} campaign contact${metrics.contacts === 1 ? '' : 's'}` : section.key === 'sequence' ? `${detail.steps.length} saved message${detail.steps.length === 1 ? '' : 's'}` : section.key === 'schedule' ? formatDateTime(campaign.startsAt) : <><span className="block">{survey.title} selected</span><span className="mt-0.5 block">From: {senderName.trim()} &lt;{campaign.senderEmail}&gt;</span></> : section.issues.join(' ')}</div></div>{section.complete ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700" aria-label="Complete" /> : <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" aria-label="Needs attention" />}</button>)}</div>{campaign.status === 'draft' ? <div><Button disabled={working || !workflowReady} onClick={() => void transition('launch')}><Send />{working ? 'Launching' : 'Launch campaign'}</Button>{!workflowReady && <p className="mt-2 text-xs text-destructive" role="status">Complete the steps marked “Needs attention” before launching.</p>}</div> : <p className="text-sm text-muted-foreground">This campaign is {campaign.status}. Its setup is retained here for reference.</p>}</CardContent></Card></TabsContent>
