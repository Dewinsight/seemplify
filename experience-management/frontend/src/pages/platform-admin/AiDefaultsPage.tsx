...
export function PlatformAdminAiDefaultsPage() {
  ...
  )) || !draft.runtimePolicy.localEnabled || !draft.runtimePolicy.chatgptEnabled || draft.runtimePolicy.defaultRuntime !== 'chatgpt'));
  ...
  return <div className="space-y-6" data-testid="platform-admin-ai-defaults">
    ...
    {!state || !draft ? !error && <AdminLoading label="Loading platform AI defaults..." /> : <>
      ...
      <section className="rounded-lg border bg-card" aria-labelledby="platform-runtime-policy-heading"><div className="flex flex-col justify-between gap-3 border-b px-5 py-4 sm:flex-row sm:items-start"><div><h2 id="platform-runtime-policy-heading" className="section-title">Runtime availability</h2><p className="mt-1 text-xs text-muted-foreground">{draft.updatedAt ? `Last changed ${formatAdminDate(draft.updatedAt)}` : 'Both runtimes are currently available.'}</p></div>{canManage && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={Boolean(working) || !hasExplicitDefaults} onClick={() => void reset()}>{working === 'reset' ? <Loader2 className="animate-spin" /> : <RotateCcw />}Reset</Button><Button size="sm" disabled={saveDisabled} onClick={() => void save()}>{working === 'save' ? <Loader2 className="animate-spin" /> : <Save />}Save policy</Button></div>}</div><div className="grid gap-4 p-5 sm:grid-cols-2"><label className="flex items-start gap-3 border p-4 text-sm"><input type="checkbox" className="mt-0.5 h-4 w-4 accent-foreground" checked={draft.runtimePolicy.localEnabled} disabled={!canManage || Boolean(working)} onChange={(event) => setDraft({ ...draft, runtimePolicy: { ...draft.runtimePolicy, localEnabled: event.target.checked } })} /><span><span className="font-medium">Local AI runtime</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Allow spaces to select the managed local runtime.</span></span></label><label className="flex items-start gap-3 border p-4 text-sm"><input type="checkbox" className="mt-0.5 h-4 w-4 accent-foreground" checked={draft.runtimePolicy.chatgptEnabled} disabled={!canManage || Boolean(working)} onChange={(event) => setDraft({ ...draft, runtimePolicy: { ...draft.runtimePolicy, chatgptEnabled: event.target.checked } })} /><span><span className="font-medium">ChatGPT / Codex</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Allow connected users to run tasks with Codex.</span></span></label><div className="sm:col-span-2"><label className="field-label" htmlFor="platform-default-runtime">Default runtime</label><select id="platform-default-runtime" className={selectClassName} value={draft.runtimePolicy.defaultRuntime} disabled={!canManage || Boolean(working) || (!draft.runtimePolicy.localEnabled && !draft.runtimePolicy.chatgptEnabled)} onChange={(event) => setDraft({ ...draft, runtimePolicy: { ...draft.runtimePolicy, defaultRuntime: event.target.value as 'local' | 'chatgpt' } })}><option value="chatgpt" disabled={!draft.runtimePolicy.chatgptEnabled}>ChatGPT / Codex</option><option value="local" disabled={!draft.runtimePolicy.localEnabled}>Local AI runtime</option></select><p className="mt-1 text-xs text-muted-foreground">Used when a space has not made an explicit choice. Existing user choices still win while their selected runtime is enabled.</p></div></div>{!draft.runtimePolicy.localEnabled && !draft.runtimePolicy.chatgptEnabled && <p className="border-t border-amber-500/35 bg-amber-50/40 px-5 py-3 text-xs leading-5 text-amber-900" role="alert">Both runtimes are off. New AI actions will be blocked until an administrator enables at least one runtime.</p>}</section>
