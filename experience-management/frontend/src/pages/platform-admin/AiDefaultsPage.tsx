...
export function PlatformAdminAiDefaultsPage() {
  ...
  const disabled = !canManage || !state?.codex.account.connected || !state.codex.models.length || Boolean(working);
  ...
  async function save() {
    if (!draft || disabled || !changed) return;
    setWorking('save');
    try {
  ...
  async function reset() {
    if (!canManage || working || !window.confirm('Clear every platform Codex default? Users without their own choices will fall back to the connected model and action defaults.')) return;
    setWorking('reset');
    try {
      ...
      toast.success('Platform Codex defaults cleared.');
    } catch (cause) {
  ...
  return <div className="space-y-6" data-testid="platform-admin-ai-defaults">
    ...
    {!state || !draft ? !error && <AdminLoading label="Loading platform AI defaults..." /> : <>
      ...
      <section className="rounded-lg border bg-card" aria-labelledby="platform-ai-default-heading"><div className="flex flex-col justify-between gap-3 border-b px-5 py-4 sm:flex-row sm:items-start"><div><h2 id="platform-ai-default-heading" className="section-title">Default configuration</h2><p className="mt-1 text-xs text-muted-foreground">{draft.updatedAt ? `Last changed ${formatAdminDate(draft.updatedAt)}` : 'No platform override has been saved.'}</p></div>{canManage && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={Boolean(working) || !hasExplicitDefaults} onClick={() => void reset()}>{working === 'reset' ? <Loader2 className="animate-spin" /> : <RotateCcw />}Clear defaults</Button><Button size="sm" disabled={disabled || !changed} onClick={() => void save()}>{working === 'save' ? <Loader2 className="animate-spin" /> : <Save />}Save defaults</Button></div>}</div>{(defaultModelUnavailable || defaultEffortUnavailable || staleActionCount > 0) && <p className="border-b border-amber-500/35 bg-amber-50/40 px-5 py-3 text-xs leading-5 text-amber-900" role="status">{defaultModelUnavailable || defaultEffortUnavailable ? 'A saved platform model or effort is no longer available to this ChatGPT account. ' : ''}{staleActionCount > 0 ? `${staleActionCount} action setting${staleActionCount === 1 ? '' : 's'} also need review. ` : ''}Choose available replacements below or clear the platform defaults.</p>}<div className="grid gap-4 p-5 sm:grid-cols-2"><div><label className="field-label" htmlFor="platform-codex-model">Default Codex model</label><select id="platform-codex-model" value={draft.codexModel || ''} disabled={disabled} onChange={(event) => changeDefaultModel(event.target.value)} className={selectClassName}><option value="">Connected account default{connectedDefaultModel ? ` (${connectedDefaultModel.displayName})` : ''}</option>{defaultModelUnavailable && <option value={draft.codexModel!}>{draft.codexModel} (unavailable)</option>}{state.codex.models.map((model) => <option value={model.id} key={model.id}>{model.displayName}{model.isDefault ? ' (account default)' : ''}</option>)}</select></div><div><label className="field-label" htmlFor="platform-codex-effort">Default reasoning effort</label><select id="platform-codex-effort" value={draft.codexReasoningEffort || ''} disabled={disabled} onChange={(event) => setDraft({ ...draft, codexReasoningEffort: event.target.value || null })} className={selectClassName}><option value="">Use each action&apos;s default</option>{defaultEffortUnavailable && <option value={draft.codexReasoningEffort!}>{effortLabel(draft.codexReasoningEffort!)} (unavailable)</option>}{defaultEfforts.map((effort) => <option value={effort} key={effort}>{effortLabel(effort)}{effort === defaultModel?.defaultReasoningEffort ? ' (model default)' : ''}</option>)}</select></div></div></section>
