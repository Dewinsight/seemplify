...
export function CampaignsPage() {
  ...
  const [surveyId, setSurveyId] = useState(requestedSurvey);
  ...
  async function load() {
    try {
      ...
      setSurveyId((current) => current || requestedSurvey || surveyRows[0]?.id || '');
  ...
  async function create(event: FormEvent) {
    ...
    if (!surveyId) return toast.error('Create or select a survey first.');
    try {
      setCreating(true);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <Card className="h-fit xl:sticky xl:top-24"><CardHeader><CardTitle>Create campaign</CardTitle></CardHeader><CardContent><form className="space-y-4" onSubmit={create}>
        ...
        <div><Label className="field-label" htmlFor="campaign-survey">Survey</Label><select id="campaign-survey" className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" value={surveyId} onChange={(event) => setSurveyId(event.target.value)} required><option value="">Select a survey</option>{surveys.map((survey) => <option value={survey.id} key={survey.id}>{survey.title} ({survey.status})</option>)}</select></div>
        <Button className="w-full" disabled={creating || !surveyId}>{creating ? <Loader2 className="animate-spin" /> : <Plus />}{creating ? 'Creating' : 'Create campaign'}</Button>
