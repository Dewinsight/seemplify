...
export interface AiJobRuntime { source: 'provider_result' | 'job_result' | 'job_snapshot' | 'legacy_default'; status: 'actual' | 'planned' | 'unknown'; provider: string | null; providerLabel: string | null; model: string | null; reasoningEffort: string | null; actionId: string | null }
export interface AiJob { id: string; kind: string; surveyId: string | null; responseId: string | null; state: 'queued' | 'processing' | 'completed' | 'failed'; stage: string; progress: number; attempt: number; input: Record<string, any>; result: any; runtime: AiJobRuntime; error: string | null; retryAt: string | null; createdAt: string; startedAt: string | null; completedAt: string | null; updatedAt: string; retry?: AiJobRetry | null; knowledgeContext?: AiJobKnowledgeContext | null }
