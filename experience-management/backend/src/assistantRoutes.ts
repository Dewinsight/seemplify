...
assistantRouter.get('/overview', async (request, response) => {
  try {
    ...
    const aiStatus = await getTerraStatus() as any;
    const runtime = {
      ready: aiStatus.ready === true,
      providerLabel: aiStatus.providerLabel || aiStatus.provider?.label || null,
      model: aiStatus.model || aiStatus.health?.model || null,
      error: aiStatus.error || (aiStatus.ready === true ? null : 'Experience AI is not ready.')
    };
