...
for (const signal of ['SIGINT', 'SIGTERM'] as const) process.on(signal, () => { aiJobRunner.stop(); void campaignRunner.stop(); void esignWorker.stop(); server.close(() => process.exit(0)); });
