...
export default function PayrollWorkflowSettingsPage() {
  ...
  return <main className="min-h-screen bg-zinc-950 px-6 py-8 text-zinc-200"><div className="mx-auto max-w-4xl">
    ...
    <section className="mt-6 border border-zinc-800 bg-zinc-900/50"><div className="border-b border-zinc-800 px-5 py-4"><h2 className="font-medium">Approval policies</h2><p className="mt-1 text-sm text-zinc-500">The submitter cannot approve when separation of duties is enabled.</p></div><div className="divide-y divide-zinc-800">{policies.map(policy => <div key={policy._id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3"><div><p className="text-sm text-zinc-200">{policy.name}{policy.isDefault ? ' · Default' : ''}</p><p className="text-xs text-zinc-500">{policy.levels.length} level{policy.levels.length === 1 ? '' : 's'} · separation of duties {policy.requireSeparationOfDuties ? 'on' : 'off'}</p></div><div className="flex gap-3">{!policy.isDefault && <button onClick={() => updatePolicy(policy._id, { isDefault: true })} disabled={!!saving} className="text-xs text-zinc-300">Make default</button>}<button onClick={() => updatePolicy(policy._id, { requireSeparationOfDuties: !policy.requireSeparationOfDuties })} disabled={!!saving} className="text-xs text-zinc-300">Toggle self-approval</button>{!policy.isDefault && policy.active && <button onClick={() => deactivate('approval-policies', policy._id)} disabled={!!saving} className="text-xs text-red-300">Remove</button>}</div></div>)}</div><form onSubmit={addPolicy} className="grid gap-3 border-t border-zinc-800 p-5 sm:grid-cols-2"><input required placeholder="Policy name" value={policyName} onChange={event => setPolicyName(event.target.value)} className="min-h-11 border border-zinc-700 bg-zinc-950 px-3 text-sm" /><label className="text-xs text-zinc-500">Approval levels<select value={approvalLevels} onChange={event => setApprovalLevels(Number(event.target.value))} className="mt-1 min-h-10 w-full border border-zinc-700 bg-zinc-950 px-3 text-sm text-zinc-200"><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></label><label className="flex items-center gap-2 text-sm text-zinc-300"><input type="checkbox" checked={approvalRequired} onChange={event => setApprovalRequired(event.target.checked)} /> Require approval</label><label className="flex items-center gap-2 text-sm text-zinc-300"><input type="checkbox" checked={separationOfDuties} onChange={event => setSeparationOfDuties(event.target.checked)} /> Block submitter self-approval</label><button disabled={!!saving} className="min-h-11 bg-amber-500 px-4 text-sm font-semibold text-zinc-950 sm:col-span-2">Add policy</button></form></section>
