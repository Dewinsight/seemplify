...
type Cycle = { _id: string; cycleNumber: string; status: string; revision: number; currentApprovalLevel: number; payPeriod: { month: number; year: number; paymentDate: string }; childRuns: Child[]; approvals: Array<{ action: string; actorName?: string; level?: number; at: string; comments?: string }>; approvalPolicyId?: { name: string; levels: Array<{ name: string; roles: string[]; minimumApprovals: number }>; requireSeparationOfDuties: boolean }; approvalCapabilities?: { canFullyApprove: boolean; canOverrideSeparationOfDuties: boolean }; reportingSummary?: { currency: string; available: boolean; totalGrossPayroll?: number; totalDeductions?: number; totalNetPayroll?: number; missingRates?: string[] }; deliveries?: Array<{ _id: string; recipientEmail: string; status: string; attemptCount: number; sentAt?: string; expiresAt: string }> };
...
export default function PayrollCyclePage() {
  ...
  return <main className="min-h-screen bg-zinc-950 px-6 py-8 text-zinc-200"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-5 border border-zinc-800 bg-zinc-900/50 p-5"><h2 className="font-medium text-zinc-100">Review and release</h2><p className="mt-1 text-sm text-zinc-500">Submitting locks this revision and totals hash. Final approval releases every successful statutory run automatically.</p>{cycle.approvalCapabilities?.canFullyApprove ? <p className="mt-2 text-xs text-zinc-400">As an organization administrator, you can submit and fully approve this cycle yourself.</p> : cycle.approvalPolicyId && <p className="mt-2 text-xs text-zinc-500">Policy: {cycle.approvalPolicyId.name} · level {cycle.currentApprovalLevel} of {cycle.approvalPolicyId.levels.length} · self-approval {cycle.approvalPolicyId.requireSeparationOfDuties ? 'blocked' : 'allowed'}</p>}<label className="mt-4 block text-sm text-zinc-400">Review note<textarea value={comments} onChange={event => setComments(event.target.value)} className="mt-1 min-h-20 w-full border border-zinc-700 bg-zinc-950 px-3 py-2 text-zinc-100" /></label><div className="mt-4 flex flex-wrap gap-3">
