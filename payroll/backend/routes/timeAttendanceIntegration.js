...
router.post('/timesheets', async (req, res) => {
  try {
    ...
    if (!Array.isArray(req.body.payCodeLines) || !req.body.payCodeLines.length) return res.status(400).json({ error: 'At least one pay-code line is required' });
    ...
    const imported = await TimeAttendanceImport.create({ ...req.body, idempotencyKey, sourcePayloadHash, supersedesImportId });
    if (supersedesImportId) await TimeAttendanceImport.updateOne({ _id: supersedesImportId }, { $set: { status: 'superseded' } });
    return res.status(202).json({ status: 'accepted', transferId: imported._id, acceptedAt: imported.acceptedAt });
  } catch (error) {
    if (error.code === 11000) {
