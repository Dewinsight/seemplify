...
async function claimIdpWebhookEvent(payload, { receiptModel, now = new Date(), leaseMs = DEFAULT_LEASE_MS } = {}) {
  ...
  if (existing.status === 'processed') return { claimed: false, duplicate: true, eventId, payloadHash: hash };
  ...
  }, { $set: { status: 'processing', leaseExpiresAt, lastError: '' } }, { new: true });
  return reclaimed
...
module.exports = { DEFAULT_LEASE_MS, payloadHash, claimIdpWebhookEvent, markIdpWebhookProcessed, markIdpWebhookFailed };
