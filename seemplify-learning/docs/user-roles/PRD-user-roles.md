...
### 3.8 Payment Collection & Withdrawal Management

#### 3.8.1 Flutterwave Payment Collection (Existing — Continues Unchanged)

> [!NOTE]
> **Codebase status — Fully implemented.** The Flutterwave payment integration is operational. These requirements document the current behavior to ensure continuity and proper documentation alongside the new user roles system.

**REQ-027a:** Course purchases must be processed via Flutterwave payment gateway (`flutterwaveService.js`)
**REQ-027b:** Payment flow lifecycle:
1. Learner clicks "Buy Course" → server creates `SimpleLmsPayment` with `status: 'initiated'`
2. Server calls `createFlutterwavePaymentLink()` → Flutterwave returns checkout URL
3. Learner redirected to Flutterwave checkout (supports card, bank transfer, USSD)
4. Learner completes payment → Flutterwave redirects to `/simple-lms/payment/verify`
5. Server calls `verifyFlutterwaveTransaction()` → verifies via Flutterwave API
6. If successful: payment updated to `status: 'successful'`, commission calculated, learner auto-enrolled

**REQ-027c:** Creator commission auto-calculated on successful payment:
- Rate resolved from `SimpleLmsCommissionSetting` (per-course > per-creator > global 70% default)
- `creatorCommissionMinor = amountMinor × (commissionRate / 100)`
- `platformShareMinor = amountMinor - creatorCommissionMinor`

**REQ-027d:** Payment verification must be idempotent — re-verifying the same transaction must not create duplicate enrollments or commissions

**REQ-027e:** If an agent referral code (`?ref=AGENT_CODE`) is present, it must persist through the Flutterwave checkout redirect and be attributed on payment success (see Section 4.6)

> [!IMPORTANT]
> **Codebase reference:** `SimpleLmsPayment` model (`src/models/SimpleLmsPayment.js`, 118 lines) tracks: `account` (buyer), `course`, `creatorAccount`, `provider: 'flutterwave'`, `txRef`, `flutterwaveTxId`, `amountMinor`, `creatorCommissionRate`, `creatorCommissionMinor`, `platformShareMinor`, `status` (enum: `initiated, pending, successful, failed, cancelled, refunded`), `verificationPayload`. The `flutterwaveService.js` (97 lines) provides `createFlutterwavePaymentLink()` and `verifyFlutterwaveTransaction()`.

#### 3.8.2 Creator Withdrawal Requests (Existing Model — Admin UI Needed)

> [!NOTE]
> **Codebase status — Model exists, admin UI incomplete.** The `SimpleLmsWithdrawal` model (`src/models/SimpleLmsWithdrawal.js`, 135 lines) is fully defined with a 5-status lifecycle: `pending → approved → paid` (or `rejected` / `cancelled`). However, the admin UI for reviewing and processing withdrawal requests needs to be built and documented.

**REQ-027f:** Creators can request withdrawal of their earned commissions
- Withdrawal request creates a `SimpleLmsWithdrawal` with `status: 'pending'`
- `payoutProfileSnapshot` is frozen from `Account.payoutProfile` at request time (bank name, account number, account name, bank code, SWIFT code, payment email, country, notes)
- Creator must have a complete payout profile before requesting withdrawal
- Amount cannot exceed available balance (total earned minus pending/approved/paid withdrawals)

**REQ-027g:** Creator can cancel a pending withdrawal (while `status: 'pending'`); status changes to `cancelled`

**REQ-027h:** Creator can view their withdrawal history with status, dates, and admin notes

#### 3.8.3 Admin Withdrawal Review & Manual Payout

> [!IMPORTANT]
> **Key process:** Payouts are **entirely manual**. The system tracks the lifecycle (pending → approved → paid) but does NOT initiate bank transfers automatically. The system admin transfers money outside the system (via bank portal, Flutterwave dashboard, or direct transfer) and then records the payout in the system.

**REQ-027i:** System admins (super_admin) can view all pending withdrawal requests in the admin dashboard, showing: creator name, email, amount, currency, requested date, and the frozen payout profile snapshot

**REQ-027j:** System admins can approve or reject withdrawal requests:
- **Approve:** Sets `status: 'approved'`, `reviewedBy: adminId`, `reviewedAt: now`; creator notified
- **Reject:** Sets `status: 'rejected'`, `adminNotes` required with reason; creator notified

**REQ-027k:** After approving, the system admin manually processes the payout:
1. Admin transfers money to the creator's bank account **outside the system**
2. Admin returns to the system and clicks "Mark as Paid"
3. Enters: transaction reference (recommended for tracking), optional admin notes
4. System updates: `status: 'paid'`, `paidBy: adminId`, `paidAt: now`, `transactionRef` stored
5. Creator notified that payout has been completed
6. **This is the final state** — no further status transitions allowed

**REQ-027l:** Withdrawal review must show the frozen `payoutProfileSnapshot` from the withdrawal record, NOT the creator's current profile (which may have changed since the request was submitted)

> [!NOTE]
> **Existing model schema (`SimpleLmsWithdrawal`):** `creatorAccount` (ObjectId), `amountMinor` (Number), `currency` (String, default NGN), `status` (5-value enum), `requestedAt` (Date), `notes` (creator, 1200 chars), `adminNotes` (admin, 3000 chars), `payoutProfileSnapshot` (object with bank details), `reviewedBy` (ObjectId), `reviewedAt` (Date), `paidBy` (ObjectId), `paidAt` (Date), `transactionRef` (String), `metadata` (Map).

#### 3.8.4 Payout Profiles for All Earning Roles

> [!IMPORTANT]
> **Codebase reference:** `Account.payoutProfile` already exists with fields: `accountName`, `accountNumber`, `bankName`, `bankCode`, `swiftCode`, `currency`, `paymentEmail`, `country`, `notes`, `updatedAt`. This same structure must be reused for agents and partners — no new model needed.

**REQ-027m:** All earning roles (Creator, Channel Sales Agent, Partner Super User, Partner User) must configure a payout profile (`Account.payoutProfile`) before they can receive payouts

**REQ-027n:** Agent payout profile setup:
- Agent configures their bank details in Agent Dashboard → Settings
- When System Admin processes an agent payout, they see the agent's payout profile (bank name, account number, account name)
- Partner Super User can also view agent payout profiles for reference
- Agent can update their payout profile at any time, but in-progress payouts use the snapshot from the time of approval

**REQ-027o:** Partner organization payout:
- Partner Super User configures the **organization's** payout profile (separate from their personal account)
- Organization payout profile stored in `Organization.partnerSettings.payoutProfile` (same field structure as `Account.payoutProfile`)
- When courses sold by the partner org generate platform share, the partner org's cut is tracked
- Partner Super User can request withdrawal of the org's earnings (similar lifecycle to creator withdrawal)

#### 3.8.5 Agent Commission Payout (Centralized Through System Admin)

> [!IMPORTANT]
> **Design decision:** All payouts — creator withdrawals, partner org withdrawals, AND agent commission payouts — are processed by the **System Admin (super_admin)**. Partner Super Users can view agent commissions and **recommend** payouts, but cannot approve or process them. This keeps all money flows centralized for trust, auditability, and simplicity.

**REQ-027p:** Agent commission payout flow:
1. `AgentSaleAttribution` records accumulate automatically per sale (no separate "request" step)
2. Partner Super User reviews agent commissions and clicks **"Recommend for Payout"** (flags the agent's pending commissions)
3. System Admin sees recommended agent payouts in the admin withdrawal queue alongside creator/partner withdrawals
4. System Admin reviews: agent name, total pending, attributed sales breakdown, agent's payout profile (bank details)
5. System Admin approves → manually transfers money to agent's bank → marks as paid
6. `AgentSaleAttribution.status`: `pending → recommended → approved → paid`

**REQ-027q:** Partner Super Users can:
- **View** all their agents' commissions, attributed sales, and payout profiles
- **Recommend** agent commissions for payout (flags them for system admin review)
- **NOT** approve or process payments — that is exclusively the system admin's responsibility

#### 3.8.6 Earnings Trace & Breakdown (All Roles)

> [!IMPORTANT]
> **Key principle:** Every withdrawal or payout request must be traceable back to the individual course sales that generated the earnings. This provides transparency for creators, agents, partners, AND the platform admin.

**REQ-027r:** Creator earnings trace — when viewing earnings or requesting a withdrawal, the creator must see:
- Individual sale breakdown: course name, buyer, sale date, sale amount, commission rate (%), commission earned
- Running total showing how the available balance was calculated
- Example trace:
  ```
  "AI Fundamentals" — Learner A — Mar 8 — ₦5,000 × 70% = ₦3,500
  "ML 101"           — Learner B — Mar 7 — ₦8,000 × 70% = ₦5,600
  "Data Science"     — Learner C — Mar 6 — ₦12,000 × 70% = ₦8,400
  ────────────────────────────────────────────────────────────────
  Total Earned: ₦17,500 | Withdrawn: ₦10,000 | Available: ₦7,500
  ```

**REQ-027s:** Agent commission trace — when viewing commissions, the agent must see:
- Individual sale breakdown: course name, buyer, sale date, sale amount, platform share, agent commission rate (%), agent commission earned
- Example trace:
  ```
  "AI Fundamentals" — via ?ref=AGENT1 — Mar 8
    Sale: ₦5,000 → Creator: ₦3,500 (70%) → Platform: ₦1,500 (30%)
    → My Commission: ₦1,500 × 10% = ₦150
  "ML 101" — via ?ref=AGENT1 — Mar 7
    Sale: ₦8,000 → Creator: ₦5,600 (70%) → Platform: ₦2,400 (30%)
    → My Commission: ₦2,400 × 10% = ₦240
  ────────────────────────────────────────────────────────────────
  Total Earned: ₦390 | Paid Out: ₦0 | Pending: ₦390
  ```

**REQ-027t:** Admin withdrawal review trace — when reviewing a creator withdrawal request, the system admin must see:
- The exact list of sales that contributed to the creator's available balance
- Each sale: course name, buyer, date, total amount, commission rate, creator's share, platform's share
- Total breakdown: total earned from all sales → total already withdrawn → amount being requested → remaining balance after this withdrawal
- This ensures the admin can verify the withdrawal amount is legitimate before approving

**REQ-027u:** Partner Super User commission review trace — when reviewing agent commissions (before recommending for payout), the partner super user must see:
- Per-agent breakdown of each attributed sale
- Each sale: course name, buyer, date, sale amount, platform share amount, agent commission rate, agent commission amount
- Agent's payout profile (bank details) for reference
- Total: pending commissions, previously recommended, previously paid

**REQ-027v:** Admin platform financial trace — the super admin financial dashboard must show the complete money flow:
- Per-sale breakdown: total amount → creator commission (%) → platform share (%) → agent commission if applicable (% of platform share) → net platform revenue
- Example trace for a sale with agent referral:
  ```
  Sale: ₦10,000 ("AI Fundamentals" — Mar 8)
  ├── Creator (Ada O.):      ₦7,000 (70%)              — Withdrawal: Pending
  ├── Platform Share:        ₦3,000 (30%)
  │   ├── Agent (Chinedu):   ₦300  (10% of platform)   — Commission: Pending
  │   └── Net Platform:      ₦2,700
...
partnerSettings: {
  ...
  payoutProfile: {                 // NEW — org-level bank details for partner withdrawal
    accountName: String,
...
- Build admin withdrawal review & approval UI (Pending / Approved-Awaiting-Payout / Paid / Rejected queues)
- Build creator earnings & withdrawal request UI (balance, request form, history)
- Build "Mark as Paid" workflow for admin (manual payout recording with transaction reference)
- Build agent commission approval & payout UI for Partner Super Users
...
13. **[New]** Should there be a minimum withdrawal amount for creators? (e.g., ₦1,000 minimum to avoid micro-withdrawal overhead for the admin team processing manual payouts)

14. **[New]** Should agent commission payouts be batched (e.g., monthly summary payout to each agent) or per-sale? **Recommendation:** Allow both — Partner Super User can approve individually or batch-approve all pending commissions for an agent.

15. **[New]** How does the partner organization itself earn revenue? Options:
    - Partner org takes a flat cut from each agent's commission (e.g., 20% of agent commission goes to org)
    - Partner org earns from course sales directly (courses they created, sold by agents or directly)
    - Partner org earns a percentage of platform share for all sales within their org
    **Recommendation:** Start with partner org earning from direct course sales (courses where `organization = partnerOrgId`). Agent commission is separate and goes directly to the agent.

16. **[New]** Should agents be able to configure their own payout profiles, or should the Partner Super User manage agent payout information centrally?
