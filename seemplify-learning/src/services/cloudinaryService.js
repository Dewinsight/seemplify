...
import { resolveStoragePlatformConfiguration } from './platformConfigurationClient.js'
...
export const uploadBufferToCloudinary = async ({
  ...
  if (policy?.configured && policy.defaultProvider === 'azure-blob') {
    ...
    const safeFolder = String(folder || 'learning').split('/').filter(Boolean)
    ...
    await blob.uploadData(buffer, { blobHTTPHeaders: { blobContentType: 'application/octet-stream' } })
    ...
    return {
      secure_url: url, public_id: storageKey, resource_type: 'blob', bytes: buffer.length,
...
export const deleteFromCloudinary = async ({ publicId, storageKey, storageProvider, provider, storageContainer, resourceType = 'raw' }) => {
  if (storageProvider === 'azure-blob' || provider === 'azure-blob') {
    ...
    return service.getContainerClient(storageContainer || configuration.containerName)
      .getBlockBlobClient(storageKey || publicId).deleteIfExists({ deleteSnapshots: 'include' })
  }
  if (!ensureCloudinaryConfigured()) {
    throw new Error('Cloudinary configuration missing')
  }
