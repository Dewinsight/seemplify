 ...
 */
export function formatMailAddress(email) {
 ...
 */
export function resolveMailTransport(environment = process.env) {
...
export function isMailConfigured(environment = process.env) {
  try {
...
export function mailTransportStatus(environment = process.env) {
  try {
 ...
 */
export async function sendMail(message, environment = process.env) {
  ...
  const payload = {
    from: message.from ? formatMailAddress(assertHeaderSafe(message.from, 'Sender address')) : formatMailAddress(transport.fromEmail),
    to,
...
export default { MAIL_SEND_PATH, MailError, formatMailAddress, isMailConfigured, isRetryableMailError, mailTransportStatus, resolveMailTransport, sendMail }
