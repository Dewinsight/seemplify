...
- [x] Exclude diGiLog Recruiter, Auto-Mailer, Rocket.Chat, Mailcow and Brevo.
...
- [x] Deploy Zulip and configure OIDC through Seemplify Identity.
...
- [x] Restore-test MongoDB, Experience PostgreSQL, Zulip PostgreSQL, Dokploy PostgreSQL, MariaDB and Redis in isolation.
- [x] Run public HTTPS, OIDC, Zulip login and authenticated external TURN smoke tests.
...
- [x] Extend the repeatable smoke suite with API health, all five OIDC launch routes, AI Interview availability, hub catalog and restart checks.
...
- [x] Remove the shared Docker-network `redis` alias collision, authenticate
  Recruiter's enrichment worker, and pin AI Interview and Zulip to their
  stack-specific Redis services.
