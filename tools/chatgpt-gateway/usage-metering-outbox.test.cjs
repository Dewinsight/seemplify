...
const secret = 'chatgpt-metering-test-secret';

function tempDirectory() {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'seemplify-chatgpt-usage-'));
...
test('outbox stores approved operational identity but never prompt or CV content', async () => {
  ...
  const outbox = new ChatGptUsageMeteringOutbox({
    directory,
...
test('exact duplicate events are idempotent but changed payloads are rejected and quarantined', async () => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'CHATGPT_USAGE_IDENTITY_CONFLICT'
  );
...
test('retry state survives a new outbox instance and 5xx never dead-letters the event', async () => {
  const directory = tempDirectory();
  const first = new ChatGptUsageMeteringOutbox({
    directory,
    endpointUrl: `https://api.example.test${DELIVERY_PATH}`,
    secret,
    initialDelayMs: 0,
  ...
  const second = new ChatGptUsageMeteringOutbox({
    directory,
    endpointUrl: `https://api.example.test${DELIVERY_PATH}`,
    secret,
    initialDelayMs: 0,
