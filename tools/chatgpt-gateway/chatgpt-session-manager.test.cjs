...
function runCodexSubjectTurn(input) {
  return complete({
    activity: input.activity || 'recruiter.general',
    chatgptSubject: input.codexSubject,
    modelCandidates: input.codexModelCandidates
      || (input.codexModel ? [{ value: input.codexModel, source: 'activity' }] : []),
    effortCandidates: input.codexEffortCandidates
      || (input.reasoningEffort ? [{ value: input.reasoningEffort, source: 'activity' }] : []),
...
test('platform API keys are stripped from a per-user session environment', () => {
  ...
  for (const forbidden of ['CODEX_API_KEY', 'OPENAI_API_KEY', 'OPENAI_BASE_URL', 'CHATGPT_GATEWAY_SHARED_SECRET']) {
...
test('a turn runs on the subject session and returns the shared Codex envelope', async () => {
  ...
  assert.equal(output.provider, 'chatgpt-connect');
...
test('per-user sessions stay off until a deployment opts in', async () => {
  ...
  try {
    ...
    }), (error) => error.code === 'CHATGPT_SESSIONS_DISABLED');
...
test('the session pool evicts the idlest session rather than growing without bound', async () => {
  ...
  try {
    delete require.cache[require.resolve('./chatgpt-session-manager.cjs')];
    const bounded = require('./chatgpt-session-manager.cjs');
