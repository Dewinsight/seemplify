...
const DELIVERY_PATH = '/api/internal/ai/v1/chatgpt-usage/events';
const TOKEN_FIELDS = [
  'inputTokens',
  'cachedInputTokens',
  'outputTokens',
...
function sanitizeUsageEvent(input = {}) {
  ...
  if (!/^usage_[a-f0-9]{48}$/.test(eventId)) {
    throw new TypeError('ChatGPT usage eventId is invalid');
  }
  if (!/^chatgptexec_[a-f0-9]{48}$/.test(gatewayExecutionId)) {
    throw new TypeError('ChatGPT gateway execution ID is invalid');
  }
  const occurredAt = new Date(input.occurredAt || Date.now());
  if (!Number.isFinite(occurredAt.getTime())) {
    throw new TypeError('ChatGPT usage event timestamp is invalid');
  }
  const event = {
    eventId,
    gatewayExecutionId,
    ...
    actorName: safeText(input.actorName, 200) || undefined,
    actorEmail: safeText(input.actorEmail, 254) || undefined,
    ...
    usageSource: safeText(input.usageSource || (input.usageReported ? 'chatgpt-gateway' : 'unreported'), 100),
  ...
  if (!event.requestId || !event.activity || !event.provider || !event.model) {
    throw new TypeError('ChatGPT usage event is missing required non-PII metadata');
  }
  return event;
}
...
function identityConflictError(eventId) {
  const error = new Error(`ChatGPT usage outbox identity conflict for ${eventId}`);
  error.code = 'CHATGPT_USAGE_IDENTITY_CONFLICT';
  return error;
}
...
class ChatGptUsageMeteringOutbox {
  constructor({
    directory,
    endpointUrl,
    secret,
    ...
    if (!directory || !endpointUrl || !secret || typeof fetchImpl !== 'function') {
      throw new TypeError('ChatGPT usage metering outbox is not fully configured');
    }
  ...
  schedule(delayMs = 0) {
    ...
    this.timer = setTimeout(() => {
      ...
      void this.flush().catch((error) => {
        this.log('error', 'ChatGPT usage metering outbox flush failed', { error: error.message });
    ...
    for (const name of names) {
      ...
      try {
        ...
        if (job.eventFingerprint && job.eventFingerprint !== eventFingerprint) {
          throw new Error('ChatGPT usage outbox job fingerprint does not match its event');
        }
        jobs.push({
          file,
          job: {
        ...
        await this.quarantineCorruptJob(file, error);
        this.log('error', 'ChatGPT usage metering outbox job could not be read', {
          file: name,
          error: error.message
        });
      }
  ...
  async quarantineIdentityConflict({
    ...
    await atomicWrite(target, JSON.stringify({
      ...
      error: 'A conflicting payload reused an existing ChatGPT usage event ID'
    }));
    this.lastErrorAt = new Date(this.now()).toISOString();
    this.lastError = `A conflicting payload reused ChatGPT usage event ${event.eventId}`;
    await this.pruneDeadLetters();
  }
  ...
  async quarantineCorruptJob(file, error) {
    ...
    this.lastErrorAt = new Date(this.now()).toISOString();
    this.lastError = 'A corrupt ChatGPT usage outbox job was quarantined';
    await this.pruneDeadLetters();
  }
  ...
  async deliver(file, job) {
    ...
    if (response.ok) {
      ...
      this.lastError = null;
      this.log('info', 'ChatGPT usage event delivered', { eventId: job.event.eventId });
    ...
    if ([400, 409, 422].includes(response.status)) {
      ...
      this.log('error', 'ChatGPT usage event was rejected permanently', {
