# Independent local inference service

This Windows-only runtime keeps Ollama on `127.0.0.1:11434` and exposes the
signed inference gateway on `127.0.0.1:11435`. CV requests use
`/v1/cv/analyze`; other catalogued AI activities use `/v1/complete`.

The gateway, rather than Recruiter or any other client application, owns:

- selected provider and model state;
- the 1–8 worker inference scheduler;
- provider and activity telemetry;
- privacy-safe request history;
- bounded durable queue-history snapshots.

The Control Center and applications are consumers. Telemetry does not fall
back to an application backend. Operational records are stored under
`.local-runtime/llm/telemetry`; prompt text, CV contents, and generated output
are never written there. Activity history is capped at 50,000 records and CV
...
- the shared service ceiling limits all concurrent work to eight workers;
- each AI activity has its own fair FIFO lane under that shared ceiling.
...
The activity lanes are short-lived execution queues. Producing applications
may retain their own job state, while the gateway permanently owns the bounded,
content-free operational history exposed to the Control Center.
