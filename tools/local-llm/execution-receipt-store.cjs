...
const EXECUTION_ID = /^localexec_[a-f0-9]{48}$/;
...
function identityConflict(executionId) {
  const error = new Error(`Local execution identity conflict for ${executionId}`);
  error.code = 'LOCAL_EXECUTION_IDENTITY_CONFLICT';
  error.status = 409;
...
function corruptReceipt(executionId, reason) {
  const error = new Error(`Local execution receipt ${executionId} is not readable${reason ? `: ${reason}` : ''}`);
  error.code = 'LOCAL_EXECUTION_RECEIPT_CORRUPT';
  error.status = 503;
  error.retryable = false;
  return error;
}
...
function ownershipError(executionId) {
  const error = new Error(`Local execution receipt lease was lost for ${executionId}`);
  error.code = 'LOCAL_EXECUTION_LEASE_LOST';
  error.status = 503;
...
function abortError(signal) {
  const error = signal?.reason instanceof Error ? signal.reason : new Error('Local execution wait was cancelled');
  if (!error.code) error.code = 'LOCAL_EXECUTION_WAIT_ABORTED';
  error.status ||= 503;
  error.retryable ??= true;
  return error;
}
...
function waitForLocalExecution(entry, signal) {
  if (signal?.aborted) return Promise.reject(abortError(signal));
  return new Promise((resolve, reject) => {
...
class LocalExecutionReceiptStore {
  constructor({
    ...
    if (!directory || !encryptionSecret) throw new TypeError('Local execution receipt store is not fully configured');
    this.directory = path.resolve(directory);
    this.key = crypto.createHash('sha256')
      .update('seemplify-local-execution-receipt-v1\0')
  ...
  receiptFile(executionId) {
    if (!EXECUTION_ID.test(String(executionId || ''))) throw new TypeError('Local gateway execution ID is invalid');
  ...
  identityIntegrity(record) {
    ...
    return crypto.createHmac('sha256', this.key)
      .update('seemplify-local-execution-identity-v1\0')
  ...
  localExecutionKey(executionId) {
    return `${this.directory}\0${executionId}`;
  }

  registerLocalExecution(executionId, requestFingerprint, ownerToken) {
    const key = this.localExecutionKey(executionId);
    let settle;
    const promise = new Promise((resolve) => { settle = resolve; });
    localExecutions.set(key, { requestFingerprint, ownerToken, promise, settle });
  }

  settleLocalExecution(executionId, ownerToken, outcome = null) {
    const key = this.localExecutionKey(executionId);
    const entry = localExecutions.get(key);
    if (!entry || entry.ownerToken !== ownerToken) return;
    localExecutions.delete(key);
  ...
  async withLock(executionId, action) {
    ...
    while (true) {
      if (await this.tryCreateOwnedLock(lockDirectory, ownerToken)) break;
      {
        ...
        if (Date.now() >= deadline) {
          const timeout = new Error(`Timed out locking local execution receipt ${executionId}`);
          timeout.code = 'LOCAL_EXECUTION_LOCK_TIMEOUT';
          timeout.status = 503;
    ...
    try {
      ...
      await this.releaseLock(lockDirectory, ownerToken).catch((error) => {
        this.log('error', 'Local execution receipt lock cleanup failed', {
          executionId,
          error: error.message
        });
      });
  ...
  encryptPrepared(executionId, requestFingerprint, state, value) {
    ...
    if (plaintext.length > this.maxPreparedBytes) {
      const error = new Error('Local execution result is too large for the durable receipt store');
      error.code = 'LOCAL_EXECUTION_RESULT_TOO_LARGE';
      error.status = 503;
      error.retryable = false;
      throw error;
    }
  ...
  expiredError(executionId) {
    const error = new Error(`Local execution receipt ${executionId} has expired; use a new execution identity`);
    error.code = 'LOCAL_EXECUTION_RECEIPT_EXPIRED';
    error.status = 410;
    error.retryable = false;
    return error;
  }
  ...
  ambiguousError(executionId) {
    const error = new Error(`Local execution ${executionId} may have reached the provider before its gateway receipt was completed`);
    error.code = 'LOCAL_EXECUTION_OUTCOME_AMBIGUOUS';
    error.status = 503;
    error.retryable = false;
    return error;
  }
  ...
  async acquire({ executionId, requestFingerprint, signal }) {
    ...
    while (true) {
      ...
      const localExecution = localExecutions.get(this.localExecutionKey(executionId));
      if (localExecution) {
        if (localExecution.requestFingerprint !== requestFingerprint) {
          this.conflicts += 1;
        ...
        const localOutcome = await waitForLocalExecution(localExecution, signal);
        if (localOutcome) {
          this.replays += 1;
      ...
      if (decision.action !== 'wait') {
        ...
        if (['execute', 'recover'].includes(decision.action)) {
          this.registerLocalExecution(executionId, requestFingerprint, decision.ownerToken);
        }
        return decision;
      }
      if (!waited) {
  ...
  async complete({ executionId, requestFingerprint, ownerToken }) {
    ...
    try {
      ...
      this.settleLocalExecution(executionId, ownerToken, prepared
  ...
  async release({ executionId, requestFingerprint, ownerToken }) {
    try {
      ...
      this.settleLocalExecution(executionId, ownerToken);
    }
  }
