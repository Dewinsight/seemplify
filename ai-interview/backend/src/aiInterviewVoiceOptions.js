...
const AI_INTERVIEW_PRICE_CENTS = 150;
...
function decorateVoiceOption(option) {
  ...
  return {
    ...
    walletSurchargeCents: tier.walletSurchargeCents,
...
function estimateAIInterviewWalletCost({
  candidateCount = 0,
  questionCount = 1,
  totalMinutes = 30,
  voiceId,
  ...
  const baseUnitPriceCents = Math.max(1, Math.ceil(Number(unitPriceCents) || AI_INTERVIEW_PRICE_CENTS));
  const voiceSurchargeCents = Math.max(0, Math.ceil(Number(voice.walletSurchargeCents) || 0));
  const finalUnitPriceCents = baseUnitPriceCents + voiceSurchargeCents;
  ...
  const unitPriceUsd = finalUnitPriceCents / 100;
  const grossMarginUsdPerCandidate = unitPriceUsd - estimatedBackendCostUsdPerCandidate;
  const grossMarginPercent = unitPriceUsd > 0 ? (grossMarginUsdPerCandidate / unitPriceUsd) * 100 : 0;
  const estimatedBackendCostUsd = estimatedBackendCostUsdPerCandidate * candidates;
  const grossMarginUsd = grossMarginUsdPerCandidate * candidates;
  const totalCents = finalUnitPriceCents * candidates;
