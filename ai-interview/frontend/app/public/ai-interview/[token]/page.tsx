...
export default function PublicAIInterviewPage() {
  ...
  const [deferredProctoringModal, setDeferredProctoringModal] = useState<ProctoringModalState | null>(null);
  ...
  const showOrDeferProctoringModal = useCallback((modal: ProctoringModalState, message?: string) => {
    const isFocusWarning = modal.severity === "warning" || modal.severity === "final";
    const candidateTurnActive = voicePhaseRef.current === "listening" || voicePhaseRef.current === "processing";
    if (isFocusWarning && candidateTurnActive) {
      setDeferredProctoringModal(modal);
      setVoiceStatus("Screen leave recorded. Finish your answer, then review the warning.");
      return;
    }
    setProctoringModal(modal);
    if (modal.severity === "input") {
      toast.warning(message || "Pasting is disabled for this interview.");
    } else if (modal.severity === "final") {
      toast.warning(message || "Final warning: leaving again will end the interview.");
    } else if (modal.severity === "blocked") {
      toast.error(message || "Interview ended due to proctoring violations.");
    } else {
      toast.warning(message || "Please keep this interview tab open.");
    }
  }, []);
  ...
  const recordProctoringEvent = useCallback(async (
    ...
    try {
      ...
      if (data.action === "terminated") {
        ...
        showOrDeferProctoringModal(buildFocusProctoringModal("terminated", data.warningMessage, nextFocusCount, nextMaxFocusViolations), data.warningMessage);
      } else if (data.action === "final_warning") {
        showOrDeferProctoringModal(buildFocusProctoringModal("final_warning", data.warningMessage, nextFocusCount, nextMaxFocusViolations), data.warningMessage);
      } else if (data.action === "warned") {
        showOrDeferProctoringModal(buildFocusProctoringModal("warned", data.warningMessage, nextFocusCount, nextMaxFocusViolations), data.warningMessage);
      } else if (category === "input" && !data.deduped) {
        showOrDeferProctoringModal(buildInputProctoringModal(type as "paste_attempt" | "drop_attempt", nextPasteCount, data.warningMessage), data.warningMessage);
      }
    } catch (error: any) {
      if (category === "input") {
        const currentPasteCount = Number(sessionRef.current?.proctoring?.pasteAttemptCount || 0);
        showOrDeferProctoringModal(buildInputProctoringModal(type as "paste_attempt" | "drop_attempt", currentPasteCount + 1), type === "drop_attempt" ? "Dropping content is disabled for this interview." : "Pasting is disabled for this interview.");
      } else {
  ...
  }, [applyPublicState, showOrDeferProctoringModal, token]);
  ...
  useEffect(() => {
    if (!deferredProctoringModal) return;
    if (voicePhase === "listening" || voicePhase === "processing" || voicePhase === "speaking") return;
    setProctoringModal(deferredProctoringModal);
    setDeferredProctoringModal(null);
    toast.warning(deferredProctoringModal.message || "Please keep this interview tab open.");
  }, [deferredProctoringModal, voicePhase]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="shrink-0 rounded-md border border-slate-200 px-2 py-0.5 text-[11px] font-medium text-slate-600">AI</span>
            </div>
