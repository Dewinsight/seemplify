  ...
  return (
    <section aria-labelledby="leave-audit-title" className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <select value={action} onChange={(event) => { setAction(event.target.value); setPage(1); }} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="">All activity</option><option value="leave_entitlement_adjusted">Entitlement changes</option><option value="leave_type_created">Leave type created</option><option value="leave_type_updated">Leave type updated</option><option value="leave_type_archived">Leave type archived</option><option value="leave_policy_updated">Policy updates</option><option value="leave_request_approved">Approvals</option></select>
      </div>
