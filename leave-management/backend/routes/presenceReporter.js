...
const { requireAuth, requireOrganization } = require('../middleware');

const router = express.Router();
const APP_ID = 'leave-management';

async function forward(req, res, pathname, allowedFields) {
  if (!req.organizationId || !req.user?.id) return res.status(409).json({ error: 'Authenticated IDP identity and organization are required' });
  const body = { appId: APP_ID, organizationId: String(req.organizationId), userId: String(req.user.id) };
