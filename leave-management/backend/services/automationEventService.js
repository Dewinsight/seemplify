...
function hubUrl() { return String(process.env.AUTOMATION_HUB_API_URL || '').replace(/\/$/, ''); }
function signingSecret() {
  const file = String(process.env.AUTOMATION_HUB_HMAC_SECRET_FILE || '').trim();
  return file ? fs.readFileSync(file, 'utf8').trim() : String(process.env.AUTOMATION_HUB_HMAC_SECRET || '').trim();
}
