...
class InvitationGatekeeper {
  ...
  getModalHTML(invitations) {
    return invitations.length === 1
      ? this.getSingleInviteModal(invitations[0])
      : this.getMultipleInvitesModal(invitations)
  }

  getSingleInviteModal(invitation) {
    const orgName = this.escapeHtml(invitation.organization?.name || 'Organization')
    const inviterName = this.escapeHtml(invitation.invitedBy?.name || invitation.invitedBy?.email || 'Unknown')
    const expiresLabel = invitation.expiresAt
      ? `<div><strong>Expires:</strong> ${this.escapeHtml(new Date(invitation.expiresAt).toLocaleDateString())}</div>`
      : ''
      ...
      <div style="background: white; border-radius: 16px; padding: 40px; max-width: 500px; width: 90%; box-shadow: 0 25px 50px rgba(0, 0, 0, 0.3);">
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; padding: 24px; margin-bottom: 24px;">
          ...
          <div style="color: rgba(255, 255, 255, 0.9); font-size: 14px; line-height: 1.6;">
            <div><strong>Role:</strong> ${this.escapeHtml(this.formatRole(invitation.role))}</div>
        ...
        <div style="text-align: center;">
          <button onclick="window.gatekeeper.accept('${invitation.id}', this, 'Accept Invitation')"
          ...
          <button onclick="window.gatekeeper.decline('${invitation.id}', this, 'Decline')"
    ...
    const invitesHTML = invitations.map((inv, index) => `
      <div style="background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 12px;">
        <div style="font-weight: 600; color: #1f2937; margin-bottom: 8px;">
          ${index + 1}. ${this.escapeHtml(inv.organization?.name || 'Organization')}
        </div>
        <div style="color: #64748b; font-size: 14px;">
          <strong>Role:</strong> ${this.escapeHtml(this.formatRole(inv.role))} &bull;
          <strong>From:</strong> ${this.escapeHtml(inv.invitedBy?.name || inv.invitedBy?.email || 'Unknown')}
        </div>
        <div style="margin-top: 12px; display: flex; gap: 8px;">
          <button onclick="window.gatekeeper.accept('${inv.id}', this, 'Accept')"
          ...
          <button onclick="window.gatekeeper.decline('${inv.id}', this, 'Decline')"
  ...
  buildOrganizationRedirect(organization) {
    if (!organization?.id) {
      return '/organizations'
    }

    const orgName = organization?.name || 'the organization'
    const successMessage = encodeURIComponent(`Welcome to ${orgName}!`)
    return `/organizations/${organization.id}/members?success=${successMessage}`
  }

  async accept(invitationId, button = null, originalLabel = 'Accept Invitation') {
    try {
      if (button) {
      ...
      alert(`Successfully joined ${organizationName}!`)
      window.location.href = this.buildOrganizationRedirect(data.organization)
    } catch (error) {
  ...
  async decline(invitationId, button = null, originalLabel = 'Decline') {
