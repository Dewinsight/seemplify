...
const clientUrl = new URL('../src/public/js/profile-personal.js', import.meta.url)
...
test('Identity personal information surface contains no payroll-owned journey steps', async () => {
  const [view, clientSource, indexSource, routeSource] = await Promise.all([
  ...
  assert.match(clientSource, /Personal information saved\./)
})
