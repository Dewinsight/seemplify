...
function readSecret() {
    ...
    return 'time-automation-development-secret';
}
function canonical({ timestamp, nonce, path, body }) { return `${timestamp}.${nonce}.POST.${path}.${JSON.stringify(body || {})}`; }
...
function createVerifier({ now = () => Date.now(), resolveSecret = readSecret, claimNonce } = {}) {
    ...
    return async function verify(req, res, next) {
        ...
        if (!Number.isFinite(timestampMs) || Math.abs(current - timestampMs) > REQUEST_TTL_MS || !/^[A-Za-z0-9_-]{16,128}$/.test(nonce) || !/^[a-f0-9]{64}$/i.test(signature)) return res.status(401).json({ error: 'Invalid automation signature', code: 'AUTOMATION_AUTH_INVALID' });
        let expected;
        ...
        try {
            if (!await claim(`time-automation:${nonce}`, Math.max(current, timestampMs) + REQUEST_TTL_MS)) return res.status(409).json({ error: 'Automation request replayed', code: 'AUTOMATION_AUTH_REPLAYED' });
