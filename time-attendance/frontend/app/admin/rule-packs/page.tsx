...
export default function RulePackStudioPage() {
    ...
    const showCoverage = async () => {
        setCoverageLoading(true); setErrorMessage('');
        try {
            const data = await rulePacksApi.coverage({ reconcile: true });
            setCoverage(data.coverage || []);
            setOptions(await rulePacksApi.assignmentOptions());
        }
    ...
    return <div className="space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2"><button onClick={syncDefaults} disabled={seeding} className="inline-flex items-center gap-2 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-100 hover:bg-zinc-800 disabled:opacity-50">{seeding ? <Loader2 className="h-4 w-4 animate-spin" /> : <FilePlus2 className="h-4 w-4" />}Publish baseline catalogue</button><button onClick={showCoverage} disabled={coverageLoading} className="inline-flex items-center gap-2 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-100 hover:bg-zinc-800 disabled:opacity-50">{coverageLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Users className="h-4 w-4" />}Assign employees</button><button onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-100 hover:bg-zinc-800"><Plus className="h-4 w-4" />New custom pack</button></div>
        </header>
...
function CoverageTable({ coverage, packs, onAssign, onClose }: any) {
    ...
    return <section className="rounded-md border border-zinc-700 bg-zinc-900">
        <div className="flex items-start justify-between border-b border-zinc-800 px-4 py-3"><div><h2 className="text-sm font-semibold text-white">Employee rule assignments</h2><p className="mt-1 text-xs text-zinc-500">{coverage.length} active {coverage.length === 1 ? 'employee' : 'employees'} synchronized from Seemplify Identity. Profile country is used automatically; an explicit assignment takes priority.</p></div><button onClick={onClose} aria-label="Close coverage"><X className="h-4 w-4 text-zinc-500" /></button></div>
