...
export default function ExceptionsPage() {
    ...
    const ownPendingReviewCount = items.filter(item => String(item.userId || item.employee?.userId) === String(user?.id)
        && ['open', 'correction_requested'].includes(item.status)).length;
    ...
    const resolve = async () => {
        if (!resolving || reviewNote.trim().length < 3) return;
        try {
            setError('');
            await exceptionsApi.resolve(resolving._id, reviewNote.trim());
            setMessage('The exception was accepted as reviewed. No worked time was added or changed.');
            setResolving(null);
            setReviewNote('');
            await load();
        } catch (requestError: any) {
    ...
    return <div className="space-y-6">
        ...
        {workspaceMode === 'management' && ownPendingReviewCount > 0 && <div role="note" className="rounded-lg border border-amber-500/30 bg-amber-500/[0.07] px-4 py-3">
            <p className="text-sm font-semibold text-amber-900 dark:text-amber-200">You cannot approve your own attendance exceptions</p>
            <p className="mt-1 text-sm leading-6 text-amber-900/80 dark:text-amber-100/80">{ownPendingReviewCount} open or requested {ownPendingReviewCount === 1 ? 'exception belongs' : 'exceptions belong'} to your employee account. A different line manager, HR Manager or Attendance Admin must review {ownPendingReviewCount === 1 ? 'it' : 'them'}. Admin access does not bypass this audit control.</p>
        </div>}
                    ...
                    return <article key={item._id} id={`exception-${item._id}`} data-testid="exception-row" className={`border-t border-[var(--suite-line)] px-5 py-4 first:border-t-0 ${focusedExceptionId === item._id ? 'bg-amber-500/[0.06]' : ''}`}>
                        <div className="grid gap-4 md:grid-cols-[minmax(170px,.8fr)_minmax(0,1.6fr)_auto] md:items-start">
                            ...
                            <div className="flex shrink-0 flex-wrap gap-2 md:justify-end">
                                ...
                                {workspaceMode === 'management' && canReviewCorrections && !isOwn && item.status === 'open' && <button onClick={() => { setResolving(item); setReviewNote(''); }} className="rounded-md border border-teal-600/40 px-3 py-1.5 text-xs font-medium text-teal-700 dark:text-teal-300">Accept exception</button>}
                                ...
                                {workspaceMode === 'management' && isOwn && ['open', 'correction_requested'].includes(item.status) && <span className="max-w-48 text-right text-xs leading-5 text-[var(--suite-muted)]">Self-review is not permitted. Another authorised reviewer must decide.</span>}
                            </div>
                        </div>
                        ...
                        {item.resolution?.note && <div className="mt-4 rounded-md border border-emerald-500/25 bg-emerald-500/[0.06] px-4 py-3 md:ml-[calc(170px+1rem)]"><p className="text-xs font-semibold text-emerald-800 dark:text-emerald-300">Accepted as reviewed{item.resolution.reviewedByName ? ` by ${item.resolution.reviewedByName}` : ''}</p><p className="mt-1 text-sm text-[var(--suite-muted)]">{item.resolution.note}</p></div>}
        ...
        {resolving && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4"><div role="dialog" aria-modal="true" aria-labelledby="resolve-title" className="w-full max-w-lg rounded-lg border border-[var(--suite-line-strong)] bg-[var(--suite-surface)] p-6 shadow-lg"><h2 id="resolve-title" className="text-base font-semibold text-[var(--suite-ink)]">Accept exception as reviewed</h2><p className="mt-2 text-sm leading-6 text-[var(--suite-muted)]">This closes the review flag and records your reason. It does not create attendance, add hours or change clock times. If the employee worked, ask them to submit corrected times instead.</p><div className="mt-4 rounded-md border border-[var(--suite-line)] bg-[var(--suite-surface-muted)] px-3 py-2 text-sm text-[var(--suite-ink)]">{formatType(resolving.type)} · {resolving.explanation?.message || resolving.description}</div><label className="mt-4 block text-sm font-medium text-[var(--suite-ink)]" htmlFor="resolution-note">Decision reason</label><textarea id="resolution-note" autoFocus value={reviewNote} onChange={event => setReviewNote(event.target.value)} rows={4} className="mt-2 w-full rounded-md border border-[var(--suite-line-strong)] bg-[var(--suite-canvas)] p-3 text-sm text-[var(--suite-ink)] outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/15" /><div className="mt-4 flex justify-end gap-2"><button onClick={() => { setResolving(null); setReviewNote(''); }} className="rounded-md px-3 py-2 text-sm font-medium text-[var(--suite-muted)] hover:bg-[var(--suite-surface-muted)]">Cancel</button><button onClick={resolve} disabled={reviewNote.trim().length < 3} className="rounded-md bg-teal-700 px-4 py-2 text-sm font-medium text-white hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-teal-600 dark:hover:bg-teal-500">Accept exception</button></div></div></div>}
