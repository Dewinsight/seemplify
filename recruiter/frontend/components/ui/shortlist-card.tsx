...
export function ShortlistCard({ jobId, jobTitle, onCandidateMoved }: ShortlistCardProps) {
  ...
  const selectAllCandidates = () => {
    const selectableIds = shortlistedCandidates
      .filter(match =>
  ...
  if (loading) {
    return (
      <Card className="glass-card border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="text-foreground">Shortlisted Candidates</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <>
      ...
      <Card className="glass-card border-0 shadow-lg">
        <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/60 dark:to-blue-800/60 text-foreground dark:text-white rounded-t-lg p-4 sm:p-6 border-b border-border/50 dark:border-white/5">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
              <CardTitle className="text-lg sm:text-xl font-semibold flex items-center gap-2 flex-wrap">
                ...
                {selectedCandidates.size > 0 && (
                  <Badge variant="secondary" className="bg-white/10 text-blue-200 border-white/10">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {shortlistedCandidates.some(match =>
                match.status !== 'rejected' &&
                match.status !== 'moved_to_pipeline' &&
                !pipelineCandidateIds.has(getCandidateId(match))
              ) && (
                  <Button
                    ...
                    variant="outline"
                    className="bg-background/90 text-blue-600 hover:bg-background border-border/50 text-sm"
                    size="sm"
                  >
                    {selectedCandidates.size > 0 ? (
                      <>
          ...
          <CardDescription className="text-muted-foreground dark:text-blue-200/80 text-sm mt-3">
            {isRanked
        ...
        <CardContent className="space-y-4 sm:space-y-6 p-3 sm:p-6">
          ...
          {shortlistedCandidates.length > 0 &&
            ...
            shortlistedCandidates.some(match =>
              ...
              <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-lg p-2 sm:p-3 mb-3 sm:mb-4 backdrop-blur-sm">
                <div className="flex items-start sm:items-center gap-2 text-xs sm:text-sm text-blue-700 dark:text-blue-200">
                  <CheckCircle className="h-3 w-3 sm:h-4 sm:w-4 mt-0.5 sm:mt-0 flex-shrink-0 text-blue-500 dark:text-blue-400" />
          ...
          {shortlistedCandidates.length > 0 ? (
              ...
              .map((match, index) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative overflow-hidden rounded-xl border transition-all duration-300 cursor-pointer backdrop-blur-md ${isSelected
                      ? 'border-blue-500/50 bg-blue-50 dark:bg-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.15)] ring-1 ring-blue-500/30'
                      : match.status === 'rejected'
                        ? 'border-red-200 dark:border-red-500/30 bg-red-50 dark:bg-red-950/10 opacity-60 hover:opacity-100'
                        : 'border-border/50 dark:border-white/5 bg-card/60 dark:bg-card/20 hover:bg-card/90 dark:hover:bg-card/40 hover:border-blue-200 dark:hover:border-white/10 hover:shadow-lg hover:scale-[1.01]'
                      }`}
                    onClick={(e) => {
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-r from-transparent via-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${match.status === 'rejected'
                      ? 'to-red-500/10'
                      : 'to-blue-500/5'
                      }`} />
                    ...
                    {match.status === 'rejected' && (
                      <div className="absolute top-4 left-4 z-10">
                        <div className="bg-red-100 dark:bg-red-500/20 backdrop-blur-md rounded-full p-2 border border-red-200 dark:border-red-500/30 shadow-lg">
                    ...
                    {match.status !== 'rejected' && !pipelineCandidateIds.has(getCandidateId(match)) && (
                      <div className="absolute top-4 left-4 z-10 transition-all duration-200 opacity-70 group-hover:opacity-100 group-hover:scale-110 group-hover:animate-pulse">
                        <div className="relative">
                          ...
                          <div className="absolute inset-0 bg-blue-500 rounded opacity-0 group-hover:opacity-30 blur-md transition-opacity"></div>
                          <Checkbox
                    ...
                    <div className="relative p-4 sm:p-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 mb-4">
                        ...
                        <div className="flex items-start gap-3 sm:gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className={`text-base sm:text-lg font-bold mb-1 transition-colors truncate ${match.status === 'rejected'
                              ? 'text-muted-foreground line-through'
                              : 'text-foreground group-hover:text-blue-600 dark:group-hover:text-blue-300'
                              }`}>
                              {getCandidateName(match)}
                            </h3>
                            <div className="flex items-center gap-2 mb-2">
                              <Briefcase className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground flex-shrink-0" />
                              <p className="text-xs sm:text-sm font-medium text-muted-foreground truncate">{getCandidatePosition(match)}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex justify-start sm:justify-end sm:ml-auto pl-[60px] sm:pl-0">
                          <Badge
                            ...
                            className={`text-xs px-2 sm:px-3 py-1 font-medium shadow-sm whitespace-nowrap ${match.status === 'moved_to_pipeline'
                              ? 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300 border-green-200 dark:border-green-500/30'
                              : match.status === 'rejected'
                                ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300 border-red-200 dark:border-red-500/30 font-bold'
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {match.candidate?.email && (
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground min-w-0">
                              <Mail className="h-3 w-3 sm:h-4 sm:w-4 text-blue-500 dark:text-blue-400 flex-shrink-0" />
                          ...
                          {match.candidate?.phone && (
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                              <Phone className="h-3 w-3 sm:h-4 sm:w-4 text-green-500 dark:text-green-400 flex-shrink-0" />
                          ...
                          {match.candidate?.location && (
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                              <MapPin className="h-3 w-3 sm:h-4 sm:w-4 text-red-500 dark:text-red-400 flex-shrink-0" />
                          ...
                          {match.candidate?.experience && (
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                              <GraduationCap className="h-3 w-3 sm:h-4 sm:w-4 text-orange-500 dark:text-orange-400 flex-shrink-0" />
                        ...
                        {match.candidate?.skills && (
                          <div className="space-y-1 sm:space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs sm:text-sm font-medium text-muted-foreground">Skills</span>
                            </div>
                            <div className="flex flex-wrap gap-1">
                              {(typeof match.candidate.skills === 'string'
                                ...
                                <Badge
                                  key={skillIndex}
                                  variant="outline"
                                  className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-purple-50 dark:bg-white/5 text-purple-700 dark:text-purple-200 border-purple-100 dark:border-white/10 hover:bg-purple-100 dark:hover:bg-white/10"
                                >
                                  {skill.trim()}
                                </Badge>
                              ))}
                      ...
                      <div className="bg-muted/30 dark:bg-black/20 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-4 border border-border/50 dark:border-white/5 backdrop-blur-sm">
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs sm:text-sm font-medium text-muted-foreground">AI Match Score</span>
                          </div>
                          {match.relevanceScore && match.relevanceScore > 0 && (
                            <Badge
                              variant="outline"
                              className={`text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 font-bold border-0 ${match.relevanceScore >= 0.8
                                ? 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300'
                                : match.relevanceScore >= 0.6
                                  ? 'bg-yellow-100 dark:bg-yellow-500/20 text-yellow-700 dark:text-yellow-300'
                                  : 'bg-muted dark:bg-white/10 text-muted-foreground dark:text-gray-300'
                                }`}
                            >
                        ...
                        {match.relevanceScore && match.relevanceScore > 0 ? (
                          <Progress
                            ...
                            className="h-2 sm:h-3 bg-muted dark:bg-white/10"
                          />
                        ) : (
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 sm:pt-4 border-t border-border/50 dark:border-white/10">
                        {/* View Profile Button */}
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => router.push(`/candidates/${getCandidateId(match)}?from=job-shortlist&jobId=${jobId}`)}
                          className="h-8 sm:h-9 px-3 bg-secondary hover:bg-secondary/80 border-border/50 dark:border-white/10 text-muted-foreground hover:text-foreground dark:text-gray-300 dark:hover:text-white text-xs sm:text-sm w-full sm:w-auto transition-colors"
                        >
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-2">
                            ...
                            {match.status === 'moved_to_pipeline' ? (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleReturnToShortlist(getCandidateId(match), getCandidateName(match))}
                              ...
                              <Button
                                size="sm"
                                onClick={() => handleMoveToPipeline(getCandidateId(match), getCandidateName(match))}
                                disabled={movingToPipeline.has(getCandidateId(match))}
                            ...
                            {match.status !== 'rejected' && !pipelineCandidateIds.has(getCandidateId(match)) && (
                              <>
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleRemoveFromShortlist(getCandidateId(match), getCandidateName(match))}
                                  disabled={removingFromShortlist.has(getCandidateId(match))}
                                  className="h-8 sm:h-9 px-2 sm:px-3 bg-red-500/10 hover:bg-red-500/20 border-red-500/30 text-red-400 hover:text-red-300"
                                >
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-base text-muted-foreground">No candidates have been shortlisted yet.</p>
              <p className="text-sm text-muted-foreground/80 mt-2">Use "Add Candidates" to start building your shortlist.</p>
            </div>
          )}
        </CardContent>
        ...
        <Dialog open={showAddCandidateDialog} onOpenChange={setShowAddCandidateDialog}>
          <DialogContent className="w-[95vw] max-w-4xl max-h-[85vh] sm:max-h-[90vh] overflow-y-auto bg-background border-border p-4 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-lg sm:text-2xl font-bold flex items-center gap-2 sm:gap-3 text-foreground">
            ...
            <div className="relative mt-2">
              ...
              <Input
                ...
                className="pl-9 text-sm sm:text-base h-9 sm:h-10 bg-secondary/50 border-input text-foreground placeholder-muted-foreground"
                value={searchQuery}
