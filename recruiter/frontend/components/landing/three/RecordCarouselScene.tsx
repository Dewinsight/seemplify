'use client';

import * as THREE from 'three';
import { useMemo, useRef, useState, type MutableRefObject } from 'react';
import { Canvas, useFrame, useThree } from '@react-three/fiber';
import { ContactShadows, Environment, Html, Lightformer, PerformanceMonitor, RoundedBox } from '@react-three/drei';
import SilkPlane from './SilkPlane';
import styles from './record-card.module.css';

/**
 * One candidate's record as it moves through the Recruiter pipeline — sourcing,
 * matching, shortlist, interviews, feedback, offer, onboarding — fanned on a
 * slow carousel. The cards are real DOM (drei <Html transform>) set in the
 * Seemplify tokens, mounted on paper-thin slabs with soft contact shadows.
 */

const CARD_W = 2.3;
const CARD_H = 1.5;
const CARD_T = 0.05;
const RING_RADIUS = 3.3;
const DOM_WIDTH = 280;
// drei maps 1 CSS px to (distanceFactor / 400) world units in transform mode.
const DISTANCE_FACTOR = (CARD_W / DOM_WIDTH) * 400;

type Tone = 'brand' | 'positive' | 'warning' | 'neutral';

interface RecordView {
  stage: string;
  tone: Tone;
  rows: Array<[string, string]>;
  status: string;
  progress?: number;
}

// One candidate, seen at every stage of the pipeline. Fictional sample data.
const CANDIDATE = { name: 'Amara Nwosu', role: 'Product designer · Lagos', initials: 'AN' };

const RECORDS: RecordView[] = [
  { stage: 'Sourced', tone: 'neutral', rows: [['Source', 'Bulk CV upload'], ['Parsed', '12 skills extracted']], status: 'CV processed' },
  { stage: 'AI match', tone: 'brand', rows: [['Match', '95% · Senior designer'], ['Evidence', 'Skills · portfolio']], status: 'Ranked #1 of 48', progress: 0.95 },
  { stage: 'Shortlist', tone: 'brand', rows: [['Pipeline', 'Stage 2 of 5'], ['Owner', 'Hiring manager']], status: 'Moved by T. Bello', progress: 0.4 },
  { stage: 'Interview', tone: 'positive', rows: [['Slot', 'Thu · 10:00 WAT'], ['Panel', '3 interviewers']], status: 'Calendar synced' },
  { stage: 'AI interview', tone: 'brand', rows: [['Format', 'Voice · 8 questions'], ['Transcript', 'Ready for review']], status: 'Notetaker complete' },
  { stage: 'Feedback', tone: 'positive', rows: [['Forms', '4 of 4 submitted'], ['Consensus', 'Strong hire']], status: 'Structured scorecard', progress: 1 },
  { stage: 'Offer', tone: 'warning', rows: [['Package', 'Approved · L4'], ['Signature', 'Pending']], status: 'Awaiting candidate' },
  { stage: 'Onboarding', tone: 'positive', rows: [['Start', 'Mon 1 Jul'], ['Plan', '30 · 60 · 90 days']], status: 'Handed to Core HR', progress: 0.2 },
];

interface Palette {
  silk: string;
  slab: string;
  track: string;
  shadow: string;
  shadowOpacity: number;
  keyLight: string;
}

const PALETTES: Record<'light' | 'dark', Palette> = {
  light: { silk: '#e2dafb', slab: '#fffdfa', track: '#7047eb', shadow: '#312d39', shadowOpacity: 0.3, keyLight: '#ffffff' },
  dark: { silk: '#3f3170', slab: '#221e2c', track: '#a98eff', shadow: '#000000', shadowOpacity: 0.34, keyLight: '#e6dfff' },
};

const easeOut = (x: number) => 1 - Math.pow(1 - x, 3);

interface RecordCardProps {
  record: RecordView;
  index: number;
  angle: number;
  palette: Palette;
  spreadRef: MutableRefObject<number>;
}

function RecordCard({ record, index, angle, palette, spreadRef }: RecordCardProps) {
  const group = useRef<THREE.Group>(null);
  const dom = useRef<HTMLDivElement>(null);
  const material = useRef<THREE.MeshStandardMaterial>(null);
  const camera = useThree((state) => state.camera);
  const scratch = useMemo(
    () => ({ normal: new THREE.Vector3(), toCamera: new THREE.Vector3(), position: new THREE.Vector3(), quaternion: new THREE.Quaternion() }),
    [],
  );

  useFrame(({ clock }) => {
    const g = group.current;
    if (!g || !dom.current) return;
    const t = clock.elapsedTime;
    const radius = RING_RADIUS + spreadRef.current * 0.7;
    g.position.set(radius * Math.cos(angle), 0, radius * Math.sin(angle));
    g.rotation.set(0, -angle + Math.PI / 2, 0);

    // How squarely this card faces the camera drives its opacity and lift.
    g.getWorldQuaternion(scratch.quaternion);
    g.getWorldPosition(scratch.position);
    scratch.normal.set(0, 0, 1).applyQuaternion(scratch.quaternion);
    scratch.toCamera.copy(camera.position).sub(scratch.position).normalize();
    const facing = scratch.normal.dot(scratch.toCamera);
    const visible = THREE.MathUtils.clamp((facing + 0.1) / 0.85, 0, 1);
    // Staggered entrance: each card rises into place over the first second.
    const intro = easeOut(THREE.MathUtils.clamp((t - 0.15 - index * 0.09) / 1.1, 0, 1));

    g.position.y = 0.16 * visible + Math.sin(t * 0.9 + angle * 3) * 0.035 - (1 - intro) * 0.6;
    g.scale.setScalar(0.88 + 0.12 * intro);
    dom.current.style.opacity = facing < 0.05 ? '0' : String((0.08 + visible * 0.92) * intro);
    dom.current.dataset.front = visible > 0.92 ? 'true' : 'false';
    if (material.current) material.current.opacity = facing < 0.05 ? 0 : (0.12 + 0.88 * visible) * intro;
  });

  return (
    <group ref={group}>
      <RoundedBox args={[CARD_W, CARD_H, CARD_T]} radius={0.07} smoothness={6}>
        <meshStandardMaterial ref={material} color={palette.slab} roughness={0.85} metalness={0} transparent />
      </RoundedBox>
      <Html transform distanceFactor={DISTANCE_FACTOR} position={[0, 0, CARD_T / 2 + 0.003]} zIndexRange={[20, 0]} style={{ pointerEvents: 'none' }}>
        <div ref={dom} className={styles.recordCard} data-tone={record.tone} style={{ width: DOM_WIDTH }}>
          <div className={styles.recordHead}>
            <span className={styles.recordAvatar}>{CANDIDATE.initials}</span>
            <div className={styles.recordWho}>
              <strong>{CANDIDATE.name}</strong>
              <small>{CANDIDATE.role}</small>
            </div>
            <span className={styles.recordStage}>{record.stage}</span>
          </div>
          <dl className={styles.recordRows}>
            {record.rows.map(([label, value]) => (
              <div key={label}>
                <dt>{label}</dt>
                <dd>{value}</dd>
              </div>
            ))}
          </dl>
          {record.progress !== undefined && (
            <div className={styles.recordBar} aria-hidden="true">
              <i style={{ width: `${Math.round(record.progress * 100)}%` }} />
            </div>
          )}
          <div className={styles.recordFoot}>
            <span className={styles.recordStatus}>
              <i />
              {record.status}
            </span>
            <span className={styles.recordId}>CND-2087</span>
          </div>
        </div>
      </Html>
    </group>
  );
}

interface CarouselProps {
  palette: Palette;
  spreadRef: MutableRefObject<number>;
  hoverRef: MutableRefObject<boolean>;
}

/** Slow carousel; the pointer nudges it, hovering slows it, scrolling spreads it. */
function Carousel({ palette, spreadRef, hoverRef }: CarouselProps) {
  const ring = useRef<THREE.Group>(null);
  const drift = useRef(0);
  const speed = useRef(0.09);

  useFrame((state, delta) => {
    if (!ring.current) return;
    speed.current = THREE.MathUtils.damp(speed.current, hoverRef.current ? 0.025 : 0.09, 2.5, delta);
    drift.current += delta * speed.current;
    const targetY = drift.current + state.pointer.x * 0.28;
    const targetX = 0.04 + state.pointer.y * -0.06 + spreadRef.current * 0.18;
    ring.current.rotation.y = THREE.MathUtils.damp(ring.current.rotation.y, targetY, 4, delta);
    ring.current.rotation.x = THREE.MathUtils.damp(ring.current.rotation.x, targetX, 4, delta);
  });

  return (
    <group ref={ring} position={[0, 0.82, 0]}>
      {RECORDS.map((record, index) => (
        <RecordCard key={record.stage} record={record} index={index} angle={(index / RECORDS.length) * Math.PI * 2} palette={palette} spreadRef={spreadRef} />
      ))}
      {/* The track the record travels along. */}
      <mesh rotation-x={-Math.PI / 2} position={[0, -CARD_H / 2 - 0.2, 0]}>
        <ringGeometry args={[RING_RADIUS - 0.012, RING_RADIUS + 0.012, 160]} />
        <meshBasicMaterial color={palette.track} transparent opacity={0.28} depthWrite={false} />
      </mesh>
    </group>
  );
}

function LookAt({ target }: { target: [number, number, number] }) {
  const camera = useThree((state) => state.camera);
  useMemo(() => camera.lookAt(...target), [camera, target]);
  return null;
}

interface RecordCarouselSceneProps {
  active?: boolean;
  theme?: 'light' | 'dark';
  compact?: boolean;
  /** 0–1: how far the hero has scrolled out of view. */
  spreadRef: MutableRefObject<number>;
  /** True while the pointer is over the hero frame. */
  hoverRef: MutableRefObject<boolean>;
}

export default function RecordCarouselScene({ active = true, theme = 'light', compact = false, spreadRef, hoverRef }: RecordCarouselSceneProps) {
  const palette = PALETTES[theme];
  const [dpr, setDpr] = useState(1.5);
  const light = theme === 'light';
  const target = useMemo<[number, number, number]>(() => [0, 0.66, 0], []);

  return (
    <Canvas
      className={styles.canvas}
      frameloop={active ? 'always' : 'never'}
      dpr={dpr}
      camera={{ position: [0, 2.3, compact ? 8.6 : 10.4], fov: 30 }}
      gl={{ antialias: true, alpha: true, powerPreference: 'high-performance' }}
      style={{ background: 'transparent' }}
    >
      <PerformanceMonitor onDecline={() => setDpr(1)} onIncline={() => setDpr(1.5)} />
      <LookAt target={target} />
      <ambientLight intensity={light ? 1.1 : 0.6} />
      <directionalLight position={[3, 6, 4]} intensity={light ? 1.5 : 1.1} color={palette.keyLight} />

      <SilkPlane color={palette.silk} lightMode={light} speed={2.2} scale={1.1} rotation={0.4} noiseIntensity={light ? 0.45 : 0.3} depth={9} offsetY={-0.9} overscan={1.6} />

      <Carousel palette={palette} spreadRef={spreadRef} hoverRef={hoverRef} />

      <ContactShadows position={[0, 0.82 - CARD_H / 2 - 0.22, 0]} opacity={palette.shadowOpacity} scale={12} blur={3.2} far={3} color={palette.shadow} />

      <Environment resolution={128}>
        <Lightformer form="circle" intensity={light ? 1.2 : 1.6} color="#ffffff" position={[-3, 5, 4]} scale={3} />
        <Lightformer form="circle" intensity={0.5} color="#c4b5fd" position={[4, 2, -5]} scale={6} />
      </Environment>
    </Canvas>
  );
}
