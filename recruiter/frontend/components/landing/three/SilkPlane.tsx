...
interface SilkPlaneProps {
  ...
  depth?: number;
  /** Vertical offset of the plane, to keep it under a camera that looks down. */
  offsetY?: number;
  /** Extra oversize so the plane covers the frame from any camera angle. */
  overscan?: number;
}
...
export default function SilkPlane({
  ...
  const uniforms = useMemo(
    ...
    // Created once; live values are pushed every frame below.
  ...
  const size = useMemo(() => {
    const view = viewport.getCurrentViewport(camera, [0, offsetY, -depth]);
    return [view.width * overscan, view.height * overscan] as const;
  }, [camera, viewport, depth, offsetY, overscan]);
  ...
  return (
    <mesh ref={mesh} position={[0, offsetY, -depth]} scale={[size[0], size[1], 1]}>
