...
function providerUsageLabel(provider: string, model?: string) {
  if (provider === 'chatgpt-codex') return "ChatGPT (user's account)";
                      ...
                      <TableBody>
                        {(settings?.routes || []).map((route) => (
                          <TableRow key={route.activity} className="border-gray-800">
                            ...
                            <TableCell className={route.provider === 'chatgpt-codex' ? 'text-blue-300' : route.provider === 'groq' ? 'text-gray-300' : 'text-green-300'}>{providerUsageLabel(route.provider, route.model)}</TableCell>
