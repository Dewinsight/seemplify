  ...
  useEffect(() => {
    const checkOrganizationStatus = async () => {
      ...
      try {
        ...
        // Only check invitations if no organization exists
        setLoadingMessage('Checking for invitations...');
        const pendingInvitations = await checkInvitations();

        // If user has invitations, go to setup page with invitation tab active
        if (pendingInvitations.length > 0) {
          console.log('📨 User has invitations, redirecting to setup with invitations');
          setLoadingMessage('Loading organization setup...');
          await new Promise(resolve => setTimeout(resolve, 500));
          router.push('/organization/setup?tab=join');
          return;
        }

        // No organization and no invitations, go to setup page with create tab
        console.log('🆕 No organization or invitations, redirecting to setup');
        setLoadingMessage('Loading organization setup...');
        await new Promise(resolve => setTimeout(resolve, 500));
        router.push('/organization/setup?tab=create');
