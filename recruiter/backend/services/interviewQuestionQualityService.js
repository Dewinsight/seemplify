...
const SPECIFICITY_TERMS = Object.freeze([
  ...
  'production', 'scale', 'stakeholder', 'trade-off', 'tradeoff', 'why'
]);
...
function buildJobSignals(job = {}) {
  const strongSource = [job.title, job.department, ...splitSkills(job.skills)].filter(Boolean);
  const contextSource = [
    ...strongSource,
    job.description,
    job.requirements,
...
function assessInterviewQuestion(question = {}, options = {}) {
  ...
  if (answerDepth < 0.8) issues.push('Expand the expected answer into evidence-based scoring guidance.');

  const criteriaScore = criteria.length >= 3 ? 1 : criteria.length === 2 ? 0.75 : criteria.length === 1 ? 0.35 : 0;
  if (criteria.length < 3) issues.push('Provide at least three distinct scoring criteria.');
  const weightTotal = criterionWeightTotal(criteria);
  if (criteria.length && Math.abs(weightTotal - 100) > 1) issues.push('Make scoring-criteria weights total 100.');

  const followUpScore = followUps.length >= 2 ? 1 : followUps.length === 1 ? 0.75 : 0;
  if (!followUps.length) issues.push('Add a probing follow-up question with a clear condition.');
  const tagScore = tags.length >= 2 ? 1 : tags.length === 1 ? 0.5 : 0;
  if (tags.length < 2) issues.push('Add at least two role-relevant skill tags.');
  ...
  if (options.expectedType && String(question.type) !== String(options.expectedType)) {
    difficultyScore *= 0.7;
    issues.push(`Use the requested ${options.expectedType} question type.`);
  }
  ...
  if (score < 0.7) blockers.push(`Raise semantic quality above 70% (current score ${Math.round(score * 100)}%).`);
  return {
    score,
...
function assessQuestionSet(questions, options = {}) {
  ...
  for (let left = 0; left < items.length; left += 1) {
    for (let right = left + 1; right < items.length; right += 1) {
      const similarity = jaccardSimilarity(items[left]?.question, items[right]?.question);
      if (similarity >= 0.72) {
        duplicateIndexes.add(left);
        duplicateIndexes.add(right);
        issues.push(`Questions ${left + 1} and ${right + 1} are too similar (${Math.round(similarity * 100)}%).`);
      }
    }
  }
